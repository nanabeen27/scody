 ...
 * 자녀가 속한 반 id.
 *
 * `buildChildReport`는 **배정 대상 행 하나로** 이 자녀의 과제를 판정한다 — 미제출(`pending`)도,
 * 그 달의 배정 수(`academySubmit`·`byWeekday`)도 같은 집합이다. 예전에는 앞은 대상 행, 뒤는 반
 * 소속이라 한 리포트 안에서 `안 낸 과제 0개`인데 `배정 4개 중 1개 냈어요`가 될 수 있었다
 * (마스터 플랜 S-013 · `__tests__/assignedTargets.test.ts`).
 */
...
describe("자녀 리포트 계산", () => {
  ...
  it("학원 과제의 날짜는 제출일에서만 온다(마감일을 대신 넣지 않는다)", () => {
    ...
    for (const row of academy) {
      ...
      const sub = seed?.submissions.find(
        (s) => s.studentId === "u_student_both",
      );
    ...
    expect(
      academy.some((row) => row.dateISO && row.dateISO < (row.dueDate ?? "")),
    ).toBe(true);
  });
  ...
  */
  it("이번 달이 비어 있어도 이번 달을 연다", () => {
    const aug = buildChildReport("u_student_parentpaid", {
      assignments: ASSIGNMENTS_SEED,
      attempts: (ATTEMPTS_SEED["u_student_parentpaid"] ?? {}) as Record<
        string,
        Attempt
      >,
  ...
  it("기록이 하나도 없으면 옮겨 갈 달도 없다", () => {
    const r = buildChildReport("u_student_academy", {
      assignments: [],
      attempts: {},
      wrongNotes: [],
      sets: SETS,
  ...
  */
  it("제출자가 기준보다 적으면 반 비교를 주지 않는다", () => {
    expect(
      classStat({
        submitters: RANK_MIN_SUBMITTERS - 1,
        rank: 2,
        avg: 70,
        mine: 80,
      }),
  ...
  it("낸 기록이 없으면 반 비교를 주지 않는다", () => {
    expect(
      classStat({ submitters: 8, rank: null, avg: 74, mine: null }),
    ).toBeNull();
  ...
  it("기한 내 제출은 제출일과 마감일이 둘 다 있는 과제만 센다", () => {
    ...
    const judgeable = r.rows.filter(
      (x) => x.source === "academy" && x.dateISO && x.dueDate,
    );
...
describe("AI 요약 다듬기", () => {
  ...
  it("이미 깔끔한 글은 그대로 둔다", () => {
    const good =
      "이번 주에 3일 공부했어요. 문학이 나아졌어요. 오늘은 과정을 칭찬해 주세요.";
