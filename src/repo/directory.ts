...
// ## 지금 호출부가 0이다 (A-154)
//
// D-184가 가입을 이메일 기준으로 바꾸면서 번호 단계와 아이디 칸이 사라졌다. 두 함수를 지우지
// 않는 이유: 확정 정책 2절이 번호를 **인증·복구·초대 확인**의 수단으로 유지하고(A-020·A-021),
// `scody_id`는 여전히 not null unique라서 계정 생성이 붙는 날 판정이 필요하다(A-153).
//
// **다만 그 판정을 이 자리로 되돌리지는 않는다.** 익명 사전 조회는 계정 열거 오라클이고
// A-100이 그 상한 없음을 이미 실측해 두었다 — 중복은 `signUp` 응답이 답해야 한다(A-152).
// 그때 이 두 함수는 지운다.
