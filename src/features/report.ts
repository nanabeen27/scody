...
export function buildChildReport(childId: string, deps: Deps, wantMonth?: string) {
   ...
   *
   * 달별 배정 수(`academySubmit`·`byWeekday`)도 같은 기준이다(S-013 해소).
   */
  ...
  */
  /*
    **대상 행이 있는 배정만 이 자녀의 것이다** — 위 `pending`과 같은 기준이다. 반 소속으로 좁히면
    나중에 그 반에 들어온 학생의 지난 배정까지 분모에 들어가는데(`addStudentsToClass`는
    `class_students`만 채우고 지난 `assignment_targets`를 만들지 않는다), 그러면 **한 리포트 안에서
    두 숫자가 어긋난다** — `안 낸 과제 0개`인데 `배정 4개 중 1개 냈어요`.
  */
  const mine = assignments.filter((a) => a.submissions.some((s) => s.studentId === childId));
  ...
  for (const a of assignments) {
    ...
    if (!a.submissions.some((s) => s.studentId === childId) || !due || monthOf(due) !== month)
      continue;
...
export function useChildReport(childId: string, month?: string): ChildReportData & ReportLoad {
  ...
  const data = useMemo(
    ...
    [childId, assignments, attempts, wrongNotes, sets, today, month, comparisons],
  );
