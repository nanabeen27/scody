...
## 2026-08-15 — 누를 수 있는 것을 눌러 보이게 · 로그인 화면 정리 · `/staff` (D-165)

### 버튼처럼 보이지 않던 버튼

소개 페이지의 `로그인`이 글자로 보인 이유는 `Button`이 아니어서가 아니라 `variant="ghost"`
(배경·테두리 없음)였기 때문이다. §8이 이미 `ghost`는 링크와 펼침에만, 진짜 명령은 `secondary`
라고 정해 둔 자리를 어기고 있었다.

같은 기준으로 `ghost` 29곳을 훑어 **진짜 명령 7개**를 `secondary`로 바꿨다:
`링크 복사` · `복사` · `제외하기`(학원 관리) · `표시 취소`(학부모) · `다시 요약하기` ·
`칭찬 보내기`(리포트) · `이 반에서 빼기`(반 상세).

**바꾸지 않은 것**(문서화된 결정): 목록 줄의 `위로`·`아래로`·`빼기`(§14-1) · 펼침
(`접기`/`더 보기`/`마감일 다시 정하기`) · `ConfirmStep`의 취소(§8 R4) · `이 문제 지우기`(§18-3) ·
`추가로 대화한 내용까지 더해서 정리하기`(§17) · 다른 화면으로 보내기만 하는 줄.

### 히어로의 두 번째 버튼을 뺐다

`이미 계정이 있어요`를 지웠다. 로그인은 상단 바에 늘 있고, 두 버튼이 나란히 서면 처음 온 사람이
무엇을 눌러야 하는지 한 번 더 판단해야 한다(§8: 화면당 주요 행동 하나).

### 테마 토글 — 위치를 다시 정했다

기획 판단: **테마는 행동이 아니라 설정**이다. 그 화면에서 하려는 일과 경쟁하면 안 된다.
예전에는 `테마 · 라이트` 라벨 버튼이 로그인 화면 **약관 문구 아래, 화면 맨 끝**에 있었다 —
로그인하러 온 사람이 끝까지 내렸을 때 마지막으로 만나는 것이 테마 전환이었다.

지금은 **상단 줄 오른쪽 끝의 아이콘 하나**다. 근거 셋: ①테마는 화면 하나가 아니라 앱 전체에
걸리는 전역 설정이라 전역 영역이 맞다 ②아이콘만 두면 본문 위계를 건드리지 않는다 ③지금 상태가
아이콘 모양으로 보인다(해·달·모니터) — 라벨이 있어야 아는 값이 아니다. 이름은
`accessibilityLabel`이 지킨다. 로그인한 뒤에는 `내 정보`의 테마 줄이 정석 자리다.

### 로그인 수단의 아이콘

- **카카오**: 공식 심볼을 쓴다. 카카오 개발자 사이트가 배포하는 로그인 버튼 이미지(600×90)를
  받아 심볼만 잘라내고 노란 배경을 투명으로 만들었다(36×34 · 471바이트 · 안티에일리어싱 보존).
  모양은 손대지 않았다. 예전에는 Feather의 범용 말풍선이라 **카카오 버튼인데 카카오 것이 아닌
  아이콘**이었다. ⚠️ 카카오 로그인은 아직 연결되지 않았다(M-DB-2) — 화면이 그 사실을 문장으로
  계속 밝힌다.
- **휴대폰**: 채운 실루엣으로 바꿨다(`PhoneMark`). 카카오가 채워진 심볼이 되자 옆의 얇은 선
  아이콘이 같은 크기인데 비어 보였다. 화면·스피커는 `fillRule="evenodd"`로 뚫어 어느 배경
  위에서도 맞다. 말풍선을 쓰지 않은 이유는 바로 옆이 카카오 말풍선이라 같은 부류로 읽히기 때문이다.

### `/staff` — 내부 로그인

카카오·휴대폰이 없어 공개 사이트에서 아무도 로그인할 수 없다. 그렇다고 계정 목록을 띄우면
seed 계정 11종(운영자 포함)이 **공개 사이트에서 원클릭**이 된다 — D-157이 닫은 구멍이다.

그래서 링크하지 않는 주소에서 **아이디와 비밀번호를 받는다.** 주소를 아는 것은 벽이 아니다.
벽은 Supabase 인증이고 비밀번호는 공개 저장소에 없는 난수다(D-157). 운영자용과 테스트용이 같은
자리를 쓴다 — 역할은 계정이 정한다. 틀렸을 때 어느 쪽이 틀렸는지 말하지 않는다.

### 부수로 배운 것 — 수명이 다른 것은 파일을 나눈다

`staffEmail`을 개발 전용 모듈(`devAccounts.ts`)에 두었더니 **그 모듈의 상수 접기가 깨졌다.**
운영 조건(`EXPO_PUBLIC_ENABLE_DEV_LOGIN=0`)으로 export한 번들에 `DEV_ACCOUNTS` 배열이 통째로
실려 `doyun`·`hanbit.teacher`·`010-1000-0003`이 되살아났다 — D-158이 없앤 것이다.

HEAD와 같은 조건으로 빌드해 대조했고(HEAD: 0회 / 변경: 1회씩), `src/session/staffEmail.ts`로
분리하니 다시 0회가 됐다. 지금 번들과 HEAD의 유일한 차이는 의도한 `@scody.test` 1회다.
...
| `npm test` | 187/187 |
| `expo export` | 성공 |
| 운영 번들 문자열 | `test1234`·`doyun`·`hanbit.teacher`·`010-1000-0003` **0회**, `@scody.test` 1회(의도) |
| `npx playwright test --project=desktop` | **166 통과 / 8 실패**(기준선과 같은 집합, 새 테스트 1건 추가) |

화면 확인: 소개(1280·390) · 로그인(390 라이트·다크) · `/staff`.

## 2026-08-18 — 로그인 전 네 화면의 UI/UX 완성도 (D-166)

범위: 소개 `/introduce`(`WebLanding.tsx`) · 로그인 `/login` · 가입 `/signup` · 내부 로그인 `/staff`
+ 공용 `AuthShell`. 기능과 데이터 흐름은 그대로 두고 완성도만 올렸다.

### 브라우저로 먼저 재어 본 것

코드만 보고 고치지 않았다. 빌드한 산출물을 띄워 DOM과 계산된 스타일을 읽었다.

| 항목 | 고치기 전 | 고친 뒤 |
|---|---|---|
| 제목(`h1`~`h6` 또는 `role="heading"`) | 소개 **0개** · 로그인 0 · 가입 0 · `/staff` 0 | 소개 **12개**(h1 1 · h2 8 · h3 3, 단계 건너뜀 없음) · 인증 화면 각 1 |
| 선택 상태 | `[aria-selected]` 3개, `[aria-pressed]` 0개 → 읽을 방법 없음 | `radiogroup` 1 + `radio` 3(`aria-checked`) |
| 44px 미달 누름 영역(소개) | 푸터 링크 24px ×4 · 테스트 계정 토글 20px · (인증) 워드마크 34px | 문서화된 예외만 남음(세그먼트 36px · `size="sm"` 32px) |
| 대비 미달(라이트) | 소개 24종 · 로그인 4 · 가입 6 · `/staff` 2 | 소개 **9종**(아이콘 글리프 3 = 비텍스트 3:1 통과 · 목업 안 6 → A-123) · 나머지 **0** |
| 대비 미달(다크) | — | 소개 3종(목업 안) · 나머지 0 |
| 좌우 넘침 | 없음 | 없음(390 · 820 · 1280) |

접근성 트리에서 실제로 읽히는 것도 확인했다:
`radiogroup > radio "학생" [checked]` · `heading "Scody는 …" [level=1]` · `heading "오늘 풀 것만 딱 …" [level=2]`.

### 고친 것

- **`AppText`에 `headingLevel`을 뒀다.** 모양은 그대로 두고 문서 구조만 말한다. react-native-web의
  `Text`는 요소를 `span`/`div`로 고정하므로 `<h1>`이 아니라 `role="heading" aria-level`로 나간다 —
  보조기술에는 같은 것이고, 태그를 얻으려 `View`로 감싸면 레이아웃이 바뀐다.
- **`title`·`display`의 줄간격 1.15 → `snug`(1.3)**, `display` 자간 -0.6 → -0.3. `DESIGN.md` §4가
  큰 한글 제목에 1.3 이상·자간 -0.2 정도까지를 요구하는데 기본값이 그것을 어겨서, 소개 페이지가
  큰 제목 **여섯 곳**에서 `fontSize`·`lineHeight`·`letterSpacing`을 손으로 다시 적어 되돌리고 있었다.
- **`SegmentedControl` → `radio`/`radiogroup` + `aria-checked`.** `aria-pressed`는 웹에서만 맞고
  react-native의 `Pressable`이 모르는 속성이라 네이티브에서 상태가 사라진다(RN은 `aria-checked`·
  `aria-selected`만 `accessibilityState`로 옮긴다).
- **읽어야 하는 문장을 `secondary`로.** `inkTertiary`는 라이트에서 2.96:1이다. 옮긴 것: 약관 초안
  고지 · `번호는 로그인과 알림에만 써요` · 카카오 프로토타입 안내 · `/staff` 안내 · 역할 설명 ·
  동의 문구 · 근거 출처 · 근거 각주 · `예시 화면` 표기 · 학원 권한 문장(`학원은 배정한 학습의 결과만
  봐요`). 목업 안의 장식 글자는 그대로 뒀다.
- **불투명도로 흐린 글자 둘을 걷어냈다.** 띠 번호 `01`(강조색 × 0.6 = **2.25:1**) → 보조 톤,
  마지막 띠 부제(흰색 × 0.9 = 4.08:1) → 그대로 흰색(4.63:1).
- **근거 수치(`설명 있는 해설 0.49 …`)는 색이 아니라 무게로 강조한다.** 강조색 13px이 `offset` 위에서
  3.85:1이었고, §3에서 `accent`는 주요 행동·선택·활성·링크의 색이다 — 인용 수치는 그 어느 것도 아니다.
- **히어로 CTA를 데스크톱에서 내용 폭으로.** 452px 청록 덩어리는 버튼보다 배너로 읽혔고, §8이
  "다른 화면으로 보내기만 하는 버튼은 절대 전폭이 아니다"라고 이미 정해 뒀다. 모바일은 전폭 유지.
- **마지막 띠 CTA를 방문자 유형과 같은 말로**(`회원가입` → `학생으로 시작하기`). 선생님으로 보고
  있으면 학원 버튼을 두지 않는다 — 주 행동이 이미 같은 곳으로 간다.
- **히어로·마지막 버튼의 `height: 52`·`radius.lg` 손복사를 `size="lg"`로.** 라벨 무게만 달랐다.
- **가입 화면의 카카오·휴대폰 아이콘을 로그인과 같은 마크로**(`KakaoSymbol`·`PhoneMark`). D-165가
  로그인만 고쳐서 **같은 카카오가 두 화면에서 다른 그림**이었다.
- 푸터 링크·워드마크·테스트 계정 토글의 누름 영역 44px(커진 만큼 음수 마진으로 되돌려 줄 높이는 그대로).
- 소개 페이지의 테스트 계정 토글을 로그인 화면과 같은 `Button`으로(맨 `Pressable` + 캡션이었다).
- `::selection`을 팔레트에서 정했다. 지문을 드래그하면 팔레트에 없는 파랑이 한 덩어리 생겼다.
- 입력에 `autoComplete`(`tel`·`name`·`username`·`new-password`·`current-password`). `/staff`는
  지금 실제로 로그인되는 유일한 자리인데 비밀번호 관리자가 채울 수 없어 32자 난수를 손으로 옮겨야 했다.
- 죽은 스타일 `footerTools` 제거(테마 토글이 상단으로 간 뒤 남아 있었다).

### 하지 않은 것

- **`inkTertiary` 토큰 값**은 건드리지 않았다 → A-123. 한 값으로 세 면 모두 4.5:1을 넘기려면
  `#8a908c` → 약 `#636966`이 되어야 하고 3단 톤 위계가 2단으로 눌린다. 팔레트 결정이 필요하다.
- **테마 기본값·저장**은 건드리지 않았다 → A-124. 기본값이 `light`인 것은 코드에 의도로 적혀 있고,
  바꾸면 다크 OS 방문자 전원의 첫인상이 바뀐다.
- **띠 번호 `01/02/03`은 남겼다.** 장식이면 뺐겠지만 세 섹션의 순서가 실제로 제품 흐름
  (문제 풀기 → 해설 → 오답노트)이고 근거 띠가 그 순서를 문장으로 말한다. 대비만 고쳤다.
- 목업 안의 글자 대비, `Row`의 `meta`, `Field`의 플레이스홀더 색 — 로그인 후 화면과 공용
  컴포넌트를 함께 바꾸는 일이라 A-123에 묶었다.

### E2E가 잡아낸 내 실수

`radio`로 바꾸자 **6건이 깨졌다.** 원인은 이름 없이 순서로 라디오를 고르던 단정이었다.

| 자리 | 무엇이 어긋났나 |
|---|---|
| `admin-flow:313` `getByRole('radio').first()` | 풀이 화면 맨 위 `5문항씩/한 문항씩` 토글이 첫 라디오가 되어 답을 고르지 못했다 |
| `student-flow:748` `getByRole('radio').nth(1)` | 같은 이유 |
| `student-flow` `[role="radio"][aria-checked="true"]` 개수 1 | 보기 방식 토글도 골라진 라디오라 2가 됐다 |
| `parent-flow` ×4 `getByRole('button', { name: '정예린' })` | 자녀 전환 세그먼트가 `radio`가 됐다 |
| `a11y:46` `getByRole('radio')` | 보기가 아닌 것을 보고도 통과할 수 있게 됐다 |

전부 **이름으로 고르도록** 고쳤다(`/보기 \d+$/`, `checked: true` 옵션). 순서로 세는 단정은
컴포넌트가 바뀌면 조용히 다른 것을 본다.

기준선을 실측으로 갈라냈다: `SegmentedControl`만 되돌려 `student-flow`를 다시 돌리니 **4 실패**,
되돌리기 전 5 실패였다 — 그 1건이 내 회귀였고 고친 뒤 다시 4가 됐다.
`parent-flow:426`은 단독 실행에서 통과해 순서 의존 플레이크로 확인했다.
...
| `npm run typecheck` | 통과 |
| `npm run lint` | 0 오류 / 5 경고(기준선) |
| `npm test` | **188/188**(세그먼트 역할 테스트 1건 추가) |
| `expo export -p web` | 성공 |
| `playwright --project=desktop` | **164 통과 / 10 실패** → 기준선 8 + 플레이크 2. 두 플레이크(`admin-flow:118` 좌석 단가 · `student-flow:859` 추가 대화)는 단독 실행에서 통과했다(각각 4.0s · 16.9s) |
| `playwright --project=tablet --project=mobile` (`auth-flow` · `a11y`) | **70/70 통과** |
| 히어로 CTA 폭 | 1280: 452 → **162** · 820: 772 → **162** · 390: 342 유지(화면이 곧 컬럼) |
| 화면 확인 | 소개·로그인·가입·`/staff` × 390·820·1280 × 라이트·다크 (좌우 넘침 0) |

기준선 8건을 실측으로 갈라 두었다: `boundary-flow:86` · `parent-flow:294`·`:541` ·
`queue-flow:224` · `student-flow:240`·`:260`·`:690`·`:810`.

**태블릿에서 한 번 틀렸다.** `hug` 기준을 `heroTwoCol`(2단 레이아웃)로 잡아서 820은 1단이지만
컬럼이 772px라 CTA가 그대로 배너였다. 스크린샷을 보고 `!isMobile`로 고쳤다 — 레이아웃 단 수가
아니라 **컬럼 폭**이 기준이다. 근거: `docs/evidence/landing-hero-cta-*.png`.

## 2026-08-18 — 학생 홈 정보 구조 재구성 (D-167)

범위: `app/student/index.tsx` 하나. 기능과 데이터 흐름은 그대로 두고 **무엇을 어디서 말하는지**만
다시 짰다. 확인은 seed 계정 셋으로 실제 화면을 띄워서 했다 — 박도윤(학원 과제 1건, 마감 지남) ·
정예린(전부 완료) · 김서준(개인 이용권만, 기록 없음).

### 재기 전에 본 것

| 계정 | 블록 수 | 카드 수 | 같은 목적지 CTA |
|---|---|---|---|
| 박도윤 | 5 (히어로 · 진행 · AI · 섹션 제목 · `위에 있는 과제 하나가 남았어요.`) | 2 | — |
| 정예린 | 5 | **4** | **2** (`개인 학습 고르기` · `문제 담으러 가기` → 둘 다 `/student/learn`) |
| 김서준 | 3 | 2 | **2** (`문제 담으러 가기`가 히어로와 빈 상태에 각각) |

도윤 화면은 **과제 1개를 네 번** 말했다. 서준(처음 온 학생)은 같은 안내를 두 번 읽고 같은
버튼을 두 개 받았다.
...
- **세 덩어리로 나눴다**: 확인할 것(칭찬 · 새 과제 · 조회 실패) → 할 일(히어로 → 진행 →
  학원 과제 → 담아 둔 학습) → 막혔을 때(AI). 덩어리가 바뀌는 자리만 간격을 24로 벌렸다
  (실측: `Screen` 컬럼 간격이 가로 여백과 같은 **16**이라 모든 경계가 같은 무게였다).
- **AI 입력창을 진행 상황과 학원 과제 사이에서 맨 아래로.** 마감 지난 과제 하나뿐인 홈에서
  두 번째로 큰 요소가 이 입력창이었다.
- **학원 과제 섹션은 히어로가 보여 주지 않은 항목이 있을 때만.** 제목도 상태 문장
  (`학원에서 내준 과제가 있어요`)에서 목록 이름(`학원에서 내준 과제`)으로 바꿨다.
- **새 과제 알림을 맨 위로.** 히어로가 대신할 수 없는 상태 변화인데 섹션 안에 있어서,
  새 과제가 히어로에 올라가면 함께 사라졌다. 칭찬과 같은 모양의 조용한 한 줄이다
  (`styles.praise` → 둘이 공유하는 `styles.notice`).
- **중복 CTA 제거.** `개인 학습 고르기`(= `home-go-learn`)를 없앴다 — `문제 담으러 가기`와
  같은 곳으로 가는 다른 이름이었다. 히어로가 그 버튼을 내놓는 상태에서는 담아 둔 학습 섹션도
  두지 않는다(`heroOffersPick`).
- **빈 상태에서 카드를 벗겼다**(`cleared` → `empty`). 선 없는 블록이 맞닿는 자리에만 §6의
  예외대로 hairline을 조건부로 둔다(`separated={academyCleared && 담아 둔 것이 0}`).
- **다 끝낸 화면의 진행 줄 라벨을 `남은 학습이 없어요` → `마친 학습`으로.** 히어로가 이미
  `오늘 할 일을 다 끝냈어요`라고 말한다 — 이 줄이 말할 것은 얼마나 했는지다.
- **한 칸짜리 `Steps`는 그리지 않는다.**

### 되돌린 것 — E2E가 정책 위반을 잡았다

출처 줄(`학원 과제 1개`)을 "쪼갤 것이 둘 이상일 때만" 그리게 고쳤다가 **되돌렸다.**
`auth-flow`(학생 홈은 남은 학습을 출처별로 센다)와 `student-flow`(진행 상황은 학원 과제와
담아 둔 학습만 센다) 둘이 동시에 깨졌고, 확정 정책 4절이 `남은 개수는 학원 과제와 개인 학습을
따로 센다`고 정해 두고 있었다. 그 줄의 일은 숫자가 아니라 **출처**다 — `남은 학습 1개`만으로는
학원이 낸 것인지 내가 담은 것인지 알 수 없다.

### 옮긴 E2E 단정 3건

사라진 중복 블록을 검사하던 자리를 **사실이 지금 있는 곳**으로 옮겼다. 확인하는 성질은 같다.

| 자리 | 전 | 후 |
|---|---|---|
| `student-flow` 학원 과제가 남았으면 알려주고 | `학원에서 내준 과제가 있어요` 제목 | 히어로가 `현대소설 점검`을 담고 있다 |
| `academy-flow` 새로 배정한 과제는 홈에 알림으로 | 같은 제목 | 히어로가 `헷갈리는 맞춤법·어법`을 담고 있다 |
| `academy-flow` 마감이 지난 미제출 과제는 | 같은 제목 | 히어로가 `현대소설 점검`을 담고 있다 |

부재 단정 2건은 새 제목으로 갱신했다(`getByText('학원에서 내준 과제', { exact: true })` —
`exact`가 없으면 `과제물을 모두 마쳤어요`에도 걸려 검사가 헛돈다).

### 44px 미달 둘 — 실측으로 찾아 고쳤다

3뷰포트 × 2테마 × 3계정(18조합)을 브라우저로 재어 보니 이 화면의 누름 영역 중 둘이 하한 미달이었다.

| 자리 | 전 | 후 | 왜 |
|---|---|---|---|
| `home-queue-all`(`전체 N개`) | **20px** | 44 | 섹션 제목 옆 링크에 최소 높이가 없었다. 커진 만큼 음수 마진으로 되돌려 제목 줄 높이는 그대로 둔다 |
| `home-queue-empty-start` | **32px**(`size="sm"`) | 44 | 같은 이름의 이 버튼이 놓인 다섯 자리 중 **빈 상태 넷은 기본 크기**이고 `sm`은 목록 아래 보조 행동에만 쓴다 — 여기만 벗어나 있었다 |

남는 것은 데스크톱 사이드바 메뉴 39px 넷(`홈`·`학습`·`기록`·`내 정보`)이다. `RoleShell`이라 모든
화면에 걸리고 마우스로 쓰는 자리라 이번 범위에서 손대지 않았다 → A-125.
## 2026-08-18 — 로그인 재기획 (D-171)

발단은 사용자 신고 두 건이다. ①휴대폰 번호로 로그인하는데 인증번호를 요구한다("가입할 때나
필요하겠지") ②`테스트 계정 보기`를 누르면 그 버튼만 사라지고 끝난다.

### 화면이 도달할 수 없는 단계를 약속하고 있었다

`app/login.tsx`가 `step: 'choose' | 'phone' | 'code'`를 선언했는데 **`setStep('code')`를 부르는
자리가 코드에 없었다.** 그래서 인증번호 `Field`·`로그인` 버튼·그 `onSubmit`이 죽은 코드였고
(`e2e/auth-flow.spec.ts`가 `login-phone-code`의 부재를 단정한다), 그러면서 화면은
`AuthSteps step={1} total={2}`로 다음 단계가 있다고 말하고 `2단계 중 1단계`로 낭독됐다.

그 반대편에는 **아이디·비밀번호로 들어갈 공개 입구가 0개**였다. 가입 화면은 그 둘을 받고
(`signup-id`·`signup-pw`) 세션에는 `signInWithScodyId`가 있고 `/staff`가 그것으로 잘 로그인한다.
D-020이 "화면에서 내린다"고 적은 수단이 실제로는 **유일하게 작동하는 로그인**이었다.

그래서 단계를 없애고 한 화면에서 자격 증명을 받는다. 서버 작업은 없었다(같은 함수를 부른다).
휴대폰은 확정 정책 2절이 이미 적어 둔 자리(인증·복구·초대 확인·연락처 변경·알림)로 돌아갔다 —
정책 안의 모순이 없어졌다.
...
| 자리 | 전 | 후 |
|---|---|---|
| 주 경로 | `휴대폰 번호로 로그인` → 번호 → **없는 인증번호 단계** | `스코디 아이디` + `비밀번호` → `로그인` |
| 진행 표시 | `2단계 중 1단계`(두 번째 단계가 없다) | 없다(한 화면이다) |
| 카카오 | 전폭 `lg` 버튼. 꺼진 빌드에서도 눌리고 안내만 나왔다 | 켠 빌드에만 버튼. 꺼진 빌드는 사실 한 줄(D-141) |
| 실패 문구 | 휴대폰 미연결 안내 + `테스트 계정 보기` 링크 | `아이디나 비밀번호를 확인해 주세요.`(어느 쪽인지 말하지 않는다) |
| `테스트 계정 보기` | 같은 라벨이 **둘**(오류 아래 링크 + 패널 토글), 링크는 두 번째부터 무반응 | 하나. 접힌 라벨이 개수를 말한다(`테스트 계정 11개 보기`) |
| 펼침 | 뷰포트 밖에서 열려 "아무 일도 없다"로 읽혔다 | 그 자리로 스크롤(`useAuthReveal`) + `aria-expanded` + `LiveRegion` |
| 이미 로그인한 사람 | `하던 학습을 이어서 할 수 있어요.` · 카카오를 누르면 다른 계정으로 갈렸다 | `이미 로그인했어요` + `내 공간으로 가기` + `다른 계정으로 로그인` |
| 복원 중 | 폼을 먼저 그렸다 | `로그인 상태를 확인하고 있어요.`(D-133) |

### 고치지 않은 것

**비밀번호 재설정은 없다(A-021).** 이 변경으로 더 급해져 P2 → P1로 올렸다 — 비밀번호를 잊은
사람은 이제 들어올 방법이 0개다. 화면에 없는 흐름을 가리키는 링크는 두지 않았다(D-141).
...
| `npm test` | 188/188 |
| `expo export -p web` | 성공 |
| `playwright --project=desktop` (전체) | **166 통과 / 8 실패** — 기준선과 **같은 집합**, 플레이크 0 |
| 재확인(`queue-flow`·`session-boundary`·`academy-flow`) | 29 통과 / 1 실패(기준선 `queue-flow:224`) |
| 좌우 넘침 | 390 · 820 · 1280 × 라이트 · 다크 × 3계정 = **18조합 모두 0** |
| 44px 미달 | 모바일·태블릿 **0**(데스크톱 사이드바 39px 넷은 범위 밖) |
| 덩어리 간격 | 항목 사이 16px · 덩어리가 바뀌는 자리 24px(실측) |

근거 이미지: `docs/evidence/home-*-before-390.png` / `home-*-after-390.png`.

## 2026-08-18 — `/simplify`(D-166 · D-167 커밋 대상) — 순 −100줄

리뷰 4관점(재사용 · 단순화 · 효율 · 고도)을 병렬로 돌려 **19건을 합치고 14건을 고쳤다.**
효율은 회귀가 없었다(오히려 렌더 작업이 줄었다 — 한 칸 `Steps`·섹션 둘·버튼 하나를 그리지 않는다).

### 가장 큰 것 — 규칙을 선언만 하고 구현하지 않았다

`headingLevel`을 **손으로 10곳**에만 붙였는데, `Screen`이 62개 화면의 제목을, `Section`이
124곳의 섹션 제목을 그린다. 그래서 `DESIGN.md` §4가 "화면 제목 1, 섹션 2"라고 선언한 뒤에도
로그인 후 화면에는 제목이 **0개**였다 — `src/theme/styles.ts`의 머리 주석이 경고하는
"지켜지지 않는 규칙을 선언한 파일" 그 자체였다.

두 줄로 186개 호출부가 해결됐다(실측):

| 화면 | 전 | 후 |
|---|---|---|
| `/student/learn` | 0개 | `h1:학습` → `h2:개인 학습` · `h2:오답노트` |
| `/student/records` | 0개 | `h1:기록` → `h2:완료한 학습` |
| `/student/queue` | 0개 | `h1:담아 둔 학습` |

### 확정 결정을 어긴 것 — 빈 상태

`담아 둔 학습이 없어요`를 손으로 다시 쌓고 있었다. **D-104는 "빈 상태는 `EmptyState` 하나다"**이고
A-094가 이미 "`records.tsx`는 `EmptyState`, `queue.tsx`는 손으로 만든 것"이라고 갈림을 적어
두었는데, 여기에 세 번째를 더한 셈이었다. `EmptyState`로 옮기니 정렬·간격·행동 자리를 컴포넌트가
정하고, 지역 스타일 둘(`empty`·`emptyAction`)이 사라졌다.

### 중복 제거

| 무엇 | 전 | 후 |
|---|---|---|
| 법률 링크 푸터 | 소개·인증 두 파일에 글자까지 같은 블록 | `src/features/legal/LegalLinks.tsx` |
| 개발용 계정 패널 | 소개·로그인 두 파일에 같은 블록(약 38줄) | `src/features/auth/DemoAccounts.tsx` |
| 텍스트 링크 44px 처리 | 5복사, 음수 값 4종 | `theme/styles.ts`의 `tap.textLine` |
| 한 칸 막대 규칙 | 호출부 삼항 + `Steps`의 `MAX` | `Steps`가 함께 갖는다(`total <= 1`) |
| `!reading && !loadError` | 4곳 | `ready` 하나 |
| `heroBtn`·`finalBtn` | 같은 값 두 이름 | `ctaPad` |
| 칭찬 확인 버튼 | 맨 `Pressable` + 44px 스타일 | `IconButton` |
| E2E 보기 로케이터 | 4스펙에 정규식 복사 | `_solve.ts`의 `choices()` |
| E2E 자녀 전환 | 4곳 + 다른 방식 4곳 | `switchChild()` |

`DemoAccounts`는 **제어형으로** 뺐다 — 로그인 화면의 오류 문구가 `테스트 계정 보기` 링크로
그 패널을 열어 주기 때문에, 상태를 안에 숨기면 그 링크가 가리킬 곳이 없어진다.
호출부가 `DEV_LOGIN_ENABLED ? … : null`로 감싸는 구조도 그대로 뒀다(상수 접기 — D-165가
`staffEmail`에서 배운 것).

### 죽은 것·틀릴 수 있던 것

- 죽은 조건: 섹션이 `!heroOffersPick`으로 이미 걸러진 안에서 `heroOffersPick ? null :`을 또 물었다.
- 학원 블록 3중 삼항 → 형제 둘(`academyList ⊂ academyTodo ⊂ academy`라 다시 묻지 않는다).
- 빈 스타일 `featureIndex: {}`, 죽은 `praiseClose`·`empty`·`emptyAction`.
- `FOCUS_CSS` → `GLOBAL_CSS`: `::selection`이 들어오면서 이름이 내용보다 좁아졌고, 새 주석이
  닫는 백틱 **아래**에 있어 아무것도 문서화하지 않고 있었다.
- `Brand`의 `marginVertical: -5`는 `(44 − 34) / 2`로 큰 워드마크에는 맞지만 **작은 것(26px)에는
  8px 틀린다.** 지금 `small`에 `onPress`를 주는 자리가 없어 드러나지 않았을 뿐이라 크기별로 갈랐다.
- 덩어리 간격이 **데스크톱에서 32**였다. `Screen`의 컬럼 간격이 모바일 16 · 그 위 24인데 고정 8을
  더해서, 주석이 주장하는 24가 모바일에서만 사실이었다. 모바일에만 더한다.

### 보류

- **로그인·가입의 인증 수단 쌍** — 추출하려면 라벨·캡션·핸들러로 prop이 6개 이상이고 두 화면의
  단계 동작이 다르다. 지금은 이르다.
- **알림 줄 지역 컴포넌트** — 한 파일에 2복사. 세 번째(조회 실패)는 모양이 달라 실제로 둘이다.
- **`useQueuedItems` 메모이제이션 · `HomeAsk` 분리** — 효율 리뷰가 "지금 측정 가능한 문제를
  건드리지 않는다"고 판정했다(카탈로그 14세트 · 키 입력마다 Map 28삽입, 마이크로초 단위).
  카탈로그가 수천 개가 되면 그때 한다.
- **사이드바 39px** — `RoleShell`이라 모든 화면에 걸린다 → A-125.

### 검증

typecheck 통과 · lint 0 오류 / 5 경고(기준선) · `npm test` 188/188 · `expo export` 성공 ·
`playwright --project=desktop` **166 통과 / 8 실패**(기준선과 **같은 집합**) ·
홈 3계정 × 390·1280 좌우 넘침 0 · 44px 미달 0(데스크톱 사이드바 제외).

`Steps`가 한 칸을 그리지 않게 되면서 `review.tsx`·`solve/[id].tsx`의 **1문항 세트**도 막대가
사라진다. 같은 이유의 같은 판단이고(옆에 `1 / 1`이 이미 있다) 두 화면 모두 E2E가 통과했다.

## 2026-08-18 — 학생 홈 네 가지: 재조회 게이트 · 칭찬 확인 · 히어로 제목 · 덩어리 간격 (D-168)

범위: `app/student/index.tsx` 하나(+ 회귀를 고정할 E2E 2건). D-167로 재구성한 화면에 남아 있던
네 가지를 고쳤다. 지적을 그대로 믿지 않고 먼저 코드에서 재현·확인했고, 넷 다 실재했다.

### ① 다시 읽는 동안 `할 일` 네 블록이 아무 문장 없이 사라졌다 [High]

- 원인: `reading = progressLoading || contentLoading`(`index.tsx`)인데 두 provider의 `loading`은
  `reading || !loaded`다(`src/features/progress.tsx:817` · `src/features/content.tsx:205`) — **재조회마다**
  참이 된다. 쓰기가 실패하면 화면을 서버 값으로 되돌리려고 `reload()`가 돌므로(`write` ·
  `dismissPraise`), 그 창에 `ready`를 쓰는 네 블록이 통째로 사라졌다: 진행 상황 · 학원 과제 섹션 ·
  `학원에서 내준 과제물을 모두 마쳤어요.` · 담아 둔 학습. 게다가 `학습을 불러오고 있어요.` 한 줄은
  히어로의 `next`가 없는 가지에만 있어서 **오늘의 학습이 있는 계정에는 문장이 하나도 없었다.**
- 고침: `loaded`를 받아 `firstLoad = !progressLoaded || !contentLoaded`로 게이트를 건다
  (`review.tsx`가 D-163으로 이미 고쳐 둔 모양). `loadError`·`ready`의 **의미는 D-136 그대로** —
  실패했을 때 개수를 세지 않고 `없어요`도 말하지 않는다. 히어로의 `불러오고 있어요` 가지도 같은 값.
- 이 구조가 성립하는 이유를 주석에 남겼다: 두 provider가 `finally`(콘텐츠는 `done`)에서 `loadedFor`를
  세팅해 **첫 조회가 실패해도 `loaded`가 참**이 되므로, 실패 줄이 정상적으로 나오고 화면이
  `불러오고 있어요`에서 영구히 멈추지 않는다.
- 실측(정예린, 1280): 칭찬 확인 PATCH를 500으로 막고 `**/rest/v1/**`의 GET을 2초 지연시킨 뒤 확인을
  누르면 — 고치기 전에는 실패 토스트가 뜬 시점에 `home-progress`·`academy-cleared`가 **둘 다 사라졌고**
  (그 자리에 문장 0건), 고친 뒤에는 둘 다 그대로 남는다. 500ms 뒤 재확인도 같다.

### ② 칭찬 `외 N개`가 한 번의 확인으로 전부 사라졌다 [High]

- 원인: 화면은 `praise[0]` 하나만 그리고 나머지를 `외 N개`로 접는데 `seePraise()`가
  `praise.map((p) => dismissPraise(p.id))`로 **미확인 전부**를 닫았다. `src/repo/parent.ts:89`가
  `.order('sent_on')` 오름차순이라 보이는 것은 가장 오래된 칭찬이고, 학생이 지난 칭찬을 다시 볼
  화면은 앱에 없다(`praiseFor` 사용처는 이 홈과 학부모 리포트뿐) — 부모가 셋을 보내면 자녀는 하나만
  읽고 나머지를 **영구히** 잃었다.
- 고침: `seePraise(id)`가 그 줄 하나만 닫는다. 실패 알림 규칙(`readOnly`면 조용히, 실패면 `show`)은
  그대로. `외 N개`는 개수가 하나씩 줄어들며 스스로 맞는다.

### ③ 히어로 제목이 §4 하한을 벗어난 손스타일이고 이 화면에는 제목이 0개였다 [High]

- 원인: `styles.heroTitle`이 `lineHeight: font.size.xxl * 1.2` · `letterSpacing: -0.4`였다 — §4는 큰
  한글 제목에 1.3 이상 · -0.2 정도까지를 요구하고 `AppText variant="title"`이 이미 그 값을 갖는다
  (D-166이 기본값을 고쳐 없앤 우회가 이 자리에 남았다). 그리고 홈은 `Screen`에 `title`을 주지 않아
  h1이 없고, 남는 제목은 조건부 섹션 둘(h2)뿐이라 처음 온 학생·과제를 다 낸 학원 학생의 홈에는
  **제목이 0개**였다(학생 탭 넷 중 홈만).
- 고침: 히어로 제목 세 자리를 `<AppText variant="title" headingLevel={1}>`로 바꾸고 `heroTitle`을
  지웠다(`WebLanding`의 동명 스타일은 별개 StyleSheet라 무관). 크기 27·무게 bold는 그대로.
- 실측(1280·820·390 × 세 상태): `[role="heading"][aria-level="1"]`이 **어느 상태에서도 정확히 1개**이고
  계산값이 `font-size 27px · line-height 35.1px · letter-spacing -0.3px`다. 320으로 좁혀 제목을 두 줄로
  만들어도 줄이 뭉치지 않는다(`docs/evidence/home-hero-title-wrap-320.png`).

### ④ 세 덩어리 간격이 모바일에서만 벌어졌다 [High]

- 원인: `askGroupGap`이 `isMobile`일 때만 `spacing.xl - spacing.lg`(8)를 더했다. `Screen`의 컬럼 간격은
  `isMobile ? lg(16) : xl(24)`(`src/components/Screen.tsx:76`)이라 모바일은 24가 됐지만
  태블릿·데스크톱은 24 + 0 — **이 화면의 다른 모든 경계와 같은 값**이었다. §14가 이름까지 붙여
  경계한 상태가 두 뷰포트에 그대로 남아 있었다.
- 고침: 폭마다 §5의 다음 토큰에 착지시킨다 — 모바일 `spacing.xl - spacing.lg`(합계 24) ·
  그 위 `spacing.xxl - spacing.xl`(합계 36).
- 실측(박도윤·정예린): 덩어리 경계 1280 **36** · 820 **36** · 390 **24**, 같은 화면의 항목 사이는
  24 · 24 · 16.

### 건드리지 않은 것

- 진행 상황의 `N / M 완료`와 `Steps`(분자가 개인 학습만 하는 학생에게 영구 0인 문제) — 결정 대기.
- `오늘 할 일을 다 끝냈어요`의 문구·조건(D-140·D-143·D-154) · 출처 내역 줄(D-167이 되돌린 자리) ·
  `새 과제가 배정되었어요` · 히어로 라벨 · `과제 N개 더 보기`.

### 검증

- `npm run typecheck` 통과 · `npm run lint` **오류 0**(기존 경고 5) · `npm test` **188건 전부 통과**(10 suite).
- `npx playwright test --project=desktop e2e/student-flow.spec.ts e2e/auth-flow.spec.ts e2e/queue-flow.spec.ts`:
  **83 passed / 5 failed**이고 실패 집합이 기준선과 **정확히 같다**(`student-flow`의 `:240`·`:260`·
  `학습이 하나도 없는 계정에는 다 끝냈다고 말하지 않는다`·`오답노트는 대화가 입력창 위에 오고` ·
  `queue-flow`의 `학원 과제는 담을 수 없다`).
- 새 E2E 1건(`parent-flow`의 `칭찬 확인은 실패하면 목록을 지우지 않고, 성공하면 한 줄만 닫는다`)을
  더한 뒤 위 셋 + `parent-flow`: **106 passed / 7 failed**. 늘어난 둘(`parent-flow:305`·`:640`)은 내 변경
  때문이 아니다 — `app/student/index.tsx`를 이번 변경 **전 상태로 되돌려 같은 두 테스트를 돌려 같은
  실패를 확인**했다(둘 다 풀이·결과 화면 단계에서 끊긴다. 기준선의 student-flow 실패 셋과 같은 갈래다).
- **①과 ②를 한 테스트에 묶었다.** 처음에는 둘로 나눴는데, 둘 다 부모 로그인 + 자녀 로그인이 필요해
  로그인이 4회 늘고 그 실행에서 `이번 주 요약을 만들면 그 주 내내 남는다`가 함께 실패했다(단독 실행은
  통과 — M-DB-15의 GoTrue 한도). 하나로 묶어 로그인 2회만 늘렸고 그 실패는 재현되지 않는다. 두 사실이
  한 흐름으로 이어지기도 한다: 확인이 실패하면 되돌아오고, 성공하면 한 줄만 닫힌다.
- 이 테스트가 **회귀를 실제로 잡는지 확인**했다. ①만 되돌리면 실패 토스트 시점의 `home-progress`
  단정에서 깨지고(`Received: false`), ②만 되돌리면 확인 뒤 남은 줄 단정(`toBeVisible`)에서 깨진다.
  둘 다 고친 상태에서만 통과한다.
- 화면 확인(1280 · 820 · 390 + 390 다크, 세 상태): `docs/evidence/home-academy-hero-{1280,820,390,390-dark}.png` ·
  `home-cleared-*.png` · `home-nothing-yet-*.png` · `home-queued-390.png` · `home-hero-title-wrap-320.png`.

### 남긴 것

- **A-126**(새로 적었다): `learn.tsx` · `pick.tsx` · `result/[id].tsx`가 아직 `loading`으로 게이트를 건다 —
  ①과 같은 사고를 들고 있다. 이번 범위가 한 파일이라 손대지 않았다.

### 검토 자체의 기록 (`/product-review` 학생 홈)

두 관점(`product-manager` 흐름·빠진 메커니즘 / `ux-auditor` 위계·클릭 가능성·문구·상태·모바일)을
병렬로 돌려 **17건을 합쳤다.** 등급을 사용자 영향으로 다시 조정한 것과 그 이유:

| 항목 | 에이전트 등급 | 조정 | 이유 |
|---|---|---|---|
| 재조회 중 네 블록 소실 | High(PM 절반) | High | UX는 이 구조를 "기준에 맞음"으로 봤다 — 재조회 창을 코드로만 보고는 놓치기 쉽다 |
| 칭찬 확인 한 번이 전부 닫음 | Medium ×2 | **High** | 부모가 보낸 내용이 **영구 소실**된다. 다시 볼 화면이 앱에 없다 |
| 히어로 손스타일 · 제목 0개 | Medium ×2 | **High** | 둘 다 레포가 **방금 선언한 규칙**(§4 · D-166)을 어긴 자리이고 모양이 바뀌지 않는 고침이다 |
| 덩어리 간격이 모바일만 | Medium | **High** | D-167이 얻으려던 것이 3뷰포트 중 1곳에서만 실현됐다 — 이번 세션이 만든 것이라 함께 닫았다 |
| 개인 전용 계정의 분자 영구 0 | UX High + PM 결정대기 | **결정 대기(M9-16)** | 두 에이전트가 **서로 다른 해법**을 냈고(오른쪽 열 감추기 vs 분자 정의 변경) §14-2·확정 정책 4절·E2E를 건드린다 |
| `오늘 할 일을 다 끝냈어요`에 시점 없음 | PM High | **결정 대기(M9-15)** | D-140·D-143·D-154를 함께 여는 일이다 |

**이미 6절에 있어 새 발견에서 뺀 것**: A-094(빈 상태 형태·`sm` 높이) · A-095(`과제 N개 더 보기` 형태) ·
A-096(이용권 없는 계정) · A-102(AI 실패) · A-105(`문제 담으러 가기` 목적지) · A-111(연결 끊은 학생) ·
A-112(히어로가 `이어서 하기`를 말하지 않음) · A-123(`inkTertiary` 대비) · A-125(사이드바 39px) ·
M-DB-8(연결 끊은 뒤 `모두 마쳤어요`가 남음).

**A-113을 닫았다** — D-167이 실제로 고쳤는데 6절에 열려 있었다. 다만 **같은 모양이 담아 둔 학습 쪽에
남아 있어** A-127로 새로 열었다.

**새로 적은 것**: A-127(담아 둔 학습 쪽 쌍둥이) · A-128(히어로 라벨 갈림) · A-129(`새 과제`가 상태를
사건으로 말함) · A-130(`다시 불러오기`의 포커스·고지 소실) · A-131(펼치면 접을 수 없음) ·
A-132(`과제물` 용어·`님` 띄어쓰기) · A-133(공개 종료로 빠진 담기). A-115에 대리 보기 칭찬 확인을 합쳤다.
...
| `npm test` | 188/188 |
| `expo export -p web` | 성공 |
| `playwright --project=desktop` (전체, 수정 뒤) | **167 통과 / 8 실패** — 기준선과 같은 집합, 통과가 1 늘었다(새 회귀 감지 테스트) |

독립 측정(3계정 × 390·820·1280, 내가 다시 잼):

| 항목 | 전 | 후 |
|---|---|---|
| `[role="heading"][aria-level="1"]` | **0개** | 세 상태 모두 **1개**(히어로 제목이 화면 상태를 이름으로 말한다) |
| 히어로 제목 | 27px / **32.4px**(1.2) / **-0.4** | 27px / **35.1px**(1.3) / **-0.3** — §4 하한 통과 |
| 덩어리 경계 | 24 / **24** / **24** | 24 / **36** / **36**(항목 사이는 16 / 24 / 24) |
| 44px 미달 | 0 | 0(데스크톱 사이드바 39px는 A-125) |
| 좌우 넘침 | 0 | 0 |

**낡은 근거 이미지**: `home-cleared-no-entitlement-*` · `home-hero-nothing-promised-*` ·
`home-overdue-hero-no-entitlement-*` 여섯 장은 **D-167 이전** 화면이다(AI 입력창이 목록 중간에 있고
`위에 있는 과제 하나가 남았어요.`가 있다). 그 시점 결정(D-141·D-143)의 근거라 지우지 않고, 지금 화면은
`home-academy-hero-*` · `home-cleared-*` · `home-nothing-yet-*`로 새로 찍었다.

## 2026-08-18 — 개선 후보 열 건을 권고안대로 닫는다 (D-169)

`/product-review`가 남긴 `다음 개선 후보` 전부. 결정 대기 둘은 기록해 둔 권장안을 그대로 적용했다.

### 결정 대기 둘

**M9-15 ② — `오늘`을 말하려면 오늘 한 일이 있어야 한다.** `오늘 할 일을 다 끝냈어요`의 실제 조건은
`약속된 일이 0개이고 과거 제출이 있다`였고 시점 개념이 없었다. 그래서 **개인 이용권 계정에는 학습을
시작한 다음 날부터 영구 상태**였다 — 마지막 제출이 이틀 전인 학생이 오늘 아무것도 하지 않아도 홈의
가장 큰 글자가 그 문장이었다. 마스터 플랜 1절(`학생은 오늘 무엇을 해야 하는지 바로 안다`)이 그
계정군에서 뒤집힌다. 이제 `doneToday`(= `attempts` 중 `dateISO === todayISO()`)로 갈라
`지금 해야 할 학습이 없어요`와 구분한다. 히어로 후보를 새로 만드는 ③은 D-140·D-034를 다시 여는
일이라 하지 않았다.

**M9-16 ① — 분자가 정의상 0인 계정에는 완료 숫자를 두지 않는다.** `rpc_submit_attempt`가 개인 학습을
제출하면 담아 둔 목록에서 그 세트를 지우므로 `goalDone = academy.length - academyTodo.length`는 학원
소속이 없는 학생에게 **항상 `0 - 0`**이다. 담은 3개 중 2개를 푼 학생이 `0 / 1 완료`와 전부 회색인
칸을 봤다. `hasAcademy`가 아니면 오른쪽 열과 칸을 그리지 않는다 — 왼쪽 두 줄이 말할 것을 다 말한다.
분자 정의를 바꾸는 ②는 확정 정책 4절의 분모 정의와 D-034를 다시 여는 일이라 하지 않았다.

### 나머지 여덟

| ID | 무엇 |
|---|---|
| A-126 | `learn.tsx` · `pick.tsx` · `result/[id].tsx`의 게이트를 `loading` → `loaded`로. D-168이 홈에서 한 것과 같다 |
| A-130 | 실패 줄을 `src/components/LoadFailed.tsx`로 뺐다(네 화면이 글자까지 같았다). 다시 시도가 도는 동안 **버튼을 언마운트하지 않고 라벨만** 갈아 포커스를 지킨다 |
| A-127 | 목록 대신 있던 `담아 둔 학습을 위에서 이어서 풀어요.` 한 줄을 없앴다 |
| A-128 | 히어로 라벨을 `오늘의 학습`으로 고정. `fromQueue`가 필요 없어졌다 |
| A-129 | `새 과제가 배정되었어요` → `아직 시작하지 않은 과제가 N개 있어요`(조건이 상태인데 문구가 사건이었다) |
| A-131 · A-095 | `과제 N개 더 보기` ↔ `접기` 한 벌 + §8 R2 형태(`secondary`·`sm`·`hug`·`tone="accent"`) |
| A-132 | `과제물` → `과제` · `{이름} 님` |
| A-133 | 담은 것이 없고 `dropped > 0`이면 빈 상태 부제가 이유를 말한다 |
| A-125 | 사이드바 메뉴 `minHeight: touch.min` — 실측 39px → 44px |

### 하다가 되돌린 것 — A-127

처음에는 학원 쪽(A-113)과 똑같이 **섹션째 감췄다**. 그러자 `queue-flow`와 `session-boundary`가
깨졌고, 원인은 **홈에서 담아 둔 학습 화면으로 가는 유일한 길이 사라진 것**이었다 — `전체 N개`가 그
진입점이고 두 테스트가 그 여정을 지키고 있었다. 없애야 하는 것은 목록 대신 있던 한 줄뿐이었다:
그 문장은 히어로를 보면 아는 사실이고, 제목과 링크는 각자 다른 일을 한다(묶음 이름 · 전체로 가는 문).
학원 쪽과 조건 모양을 맞추는 것보다 **각 요소가 하는 일을 세는 것**이 옳았다.

### 문구를 바꿔 고친 E2E 단정 (무엇을 왜)

| 자리 | 전 | 후 | 왜 |
|---|---|---|---|
| `auth-flow` 테스트 계정으로 로그인한다 | `/박도윤님/` | `/박도윤 님/` | A-132(존칭 띄어쓰기) |
| `student-flow` 학원 과제 완료 | `과제물을 모두 마쳤어요.` | `과제를 모두 마쳤어요.` | A-132(용어) |
| `academy-flow` ×2 | `새 과제가 배정되었어요` | `아직 시작하지 않은 과제가 있어요` | A-129 |
| `student-flow` ×2 · `queue-flow` ×1 | `today-primary` 안 `담아 둔 학습` | 같은 자리 `개인 학습` | A-128 — 계약을 라벨에서 **출처 태그**로 옮겼다 |
| `student-flow` 진행 상황 | `/ 1 완료` 보임 | `/ N 완료` 0개 | M9-16 ① |
| `parent-flow` 칭찬 자리 | `오늘 할 일을 다 끝냈어요`의 좌표 | **1단계 제목**의 좌표 | M9-15로 문장이 갈리므로 날짜에 매달리지 않게. D-168의 h1을 쓴다 |

**새 테스트 1건**: `student-flow`의 `오늘 낸 것이 없으면 오늘 다 끝냈다고 말하지 않는다` — 정예린
(제출이 모두 과거)에게 `지금 해야 할 학습이 없어요`가 보이고, 오늘 한 세트를 내면 같은 자리가
`오늘 할 일을 다 끝냈어요`로 바뀌는 것을 함께 확인한다. M9-15가 되돌아가면 이 테스트가 깨진다.
...
| `npm test` | 188/188 |
| `expo export -p web` | 성공 |
| `playwright --project=desktop` | **168 통과 / 8 실패** — 기준선과 **같은 집합**, 통과 +2(D-168·D-169가 각 1건 추가) |

화면 확인 — 3계정 × 390·820·1280 × 라이트·다크 = **18조합 전부**:

| 항목 | 결과 |
|---|---|
| `[role="heading"][aria-level="1"]` | 모두 **1개** |
| 44px 미달 | **0**(사이드바 39px가 A-125로 닫혀 데스크톱에도 없다) |
| 좌우 넘침 | 0 |
| 히어로 문장 | 박도윤 `현대소설 점검` · 정예린 **`지금 해야 할 학습이 없어요`** · 김서준 `아직 시작한 학습이 없어요` |

정예린의 히어로가 `오늘 할 일을 다 끝냈어요`에서 바뀐 것이 M9-15의 실측 근거다 — seed의 제출이
모두 과거이므로 오늘 한 일이 없다. 근거 이미지: `docs/evidence/home-{doyun,yerin,seojun}-d169-390.png` ·
`home-yerin-d169-390-dark.png`.
| `npm run lint` | 0 오류 / 4 경고(기준선과 같음 — `fonts.web.ts`의 `require`) |
| `npm test` | 187/187 (기준선 187, 같은 조건으로 대조) |
| `expo export -p web` | 성공(2.7MB 번들) |
| 운영 번들 문자열(`ENABLE_DEV_LOGIN=0`, `\uXXXX` 디코드 후) | `한빛 원장`·`스코디 관리자`·`doyun`·`010-3000-0001` 0회 · `테스트 계정` 13 · `개발용 계정이에요` 2 · `@scody.test` 1 — **HEAD와 같은 값**(D-135·D-145·D-158의 접기 유지) |
| Playwright | **돌리지 않았다**(원격 Supabase를 운영 사이트와 공유한다 — M-DB-7). 스펙 변경은 아래 표에 남겼다 |

### E2E 단정 변경

| 파일 | 무엇 | 왜 |
|---|---|---|
| `auth-flow.spec.ts` | `휴대폰 인증은 아직 연결되지 않았다고 말한다` → `아이디와 비밀번호로 로그인한다` | 그 단계가 화면에서 없어졌다. 새 스펙은 실제로 들어가는 것을 확인한다 |
| `auth-flow.spec.ts` | `연결되지 않았다는 오류에서 테스트 계정으로 갈 수 있다` → `로그인이 틀리면 어느 쪽이 틀렸는지 말하지 않는다` + `테스트 계정 패널은 개수를 밝히고 펼치면 목록이 보인다` | 오류가 가리키던 링크를 없앴다(같은 라벨 둘). 대신 신고된 두 결함을 각각 고정한다 |
| `auth-flow.spec.ts` | `키보드만으로 번호 단계를 끝낼 수 있다(Enter)` → `키보드만으로 로그인할 수 있다(Enter)` | Enter가 닿는 자리가 `인증번호 받기`에서 `로그인`으로 바뀌었다 |
| `auth-flow.spec.ts` | `가입되지 않은 번호 오류에서 바로 회원가입으로 간다` → `이미 로그인한 사람에게는 내 공간으로 가는 길을 준다` | 앞의 단정은 새 실패 문구 스펙과 겹친다. 검사가 없던 새 갈래로 바꿨다 |
| `auth-flow.spec.ts` · `queue-flow.spec.ts` · `student-flow.spec.ts` | 로그인 화면 도착 표시 `login-phone` → `login-submit`(4곳) | 그 testID가 없어졌다. 단정의 뜻(로그인으로 가드된다)은 그대로다 |
| `a11y.spec.ts` | 로그인 폼 라벨 검사를 아이디·비밀번호로. `aria-expanded` 검사 추가 | 폼이 바뀌었다. 펼침 상태가 보조기술에 읽히는지도 함께 고정한다 |
| `_auth.ts` | `PHONE_PENDING` 삭제 · 머리말과 `PHONE_BY_ID` 주석 갱신 | 화면에 없는 문구다. `PHONE_BY_ID`는 가입 쪽 중복 검사에서만 쓴다 |

## 2026-08-18 — 모든 페이지를 '의도'로 걸어 본 전수 감사와 20개 병렬 재기획 (D-170 · D-172~D-175)

발단은 사용자가 로그인 화면에서 발견한 두 가지다.

1. **휴대폰 번호로 로그인하려는데 인증번호를 요구한다** — "가입할 때나 필요하겠지".
2. **`테스트 계정 보기`를 누르면 그 버튼만 사라지고 끝난다.**

둘 다 재현했고 둘 다 실재했다. 그리고 파 보니 사용자가 본 것보다 컸다 — `app/login.tsx`가
`step: 'choose' | 'phone' | 'code'`를 선언하는데 **`setStep('code')`를 부르는 자리가 코드에 없어서**
인증번호 `Field`·`로그인` 버튼·그 `onSubmit`이 죽은 코드였고, 그러면서 `AuthSteps step={1} total={2}`가
"다음 단계가 있다"고 말하고 `2단계 중 1단계`로 낭독됐다. 반대편에는 **가입 화면이 정한 아이디·비밀번호로
들어갈 공개 입구가 0개**였다(`signInWithScodyId`는 이미 있고 `/staff`가 그것으로 로그인한다).
`테스트 계정 보기`는 실제로 펼쳐지는데 펼쳐진 11행이 뷰포트 밖이라 "아무 일도 없다"로 읽혔고,
같은 라벨이 오류 아래 링크와 패널 토글로 **한 화면에 둘** 있었으며 그중 하나는 두 번째부터 no-op였다.

그래서 사용자 지시대로 모든 페이지에 의도를 들고 접근해 조사 에이전트 셋으로 훑었다(약 60건).
20개 유닛으로 쪼개 격리된 git 워크트리에서 병렬로 다시 기획했다.

### 유닛 20개

| # | 유닛 | 핵심 |
|---|---|---|
| 1 | 로그인 재기획 | 아이디+비밀번호 한 화면, 죽은 `code` 단계 제거, 중복 라벨 하나로, 펼침을 그 자리로 스크롤, 이미 로그인한 사람 처리 |
| 2 | 가입 재기획 | 막히는 사실을 3단계 끝이 아니라 첫 화면에서, `DEV_LOGIN_ENABLED`로 갈라 없는 것을 가리키지 않게, `?role=parent` 인정 |
| 3 | 초대 흐름 | 학부모 분기에 누를 것이 0개였던 것과 만료·이미 사용·없는 토큰이 문구와 다른 곳으로 보내던 것 |
| 4 | 랜딩·진입 정직성 | `가입은 잠깐이면 돼요`를 CTA 전에 사실로, 역할별 CTA가 `?role=`을 싣게 |
| 5 | 문서 복귀·공간 전환 | `/legal/*`이 항상 `/introduce`로 가던 것(앱에서는 로그인으로 튄다)을 온 곳으로, `/select-space`에 이탈 경로 |
| 6 | 학생 홈 히어로 | 히어로 후보를 마감 우선으로, `이어서 풀기` + `N / M 풀었어요` |
| 7 | 기록 탭 | 출처·날짜·출처 필터, 이용권 없는 계정의 죽은 버튼 |
| 8 | 오답노트 | 미정리·별표 필터, 다섯 개 상한, 추천을 마무리 위로, 사라지는 대화 고지 |
| 9 | 카드 복습 | 해설을 정답 줄 아래에(docblock은 보여준다고 적었는데 한 번도 렌더하지 않았다) |
| 10 | 결과·상세·풀이 | 지난 마감을 `dueLabel`로, 늦은 제출 문장, 결과에서 다음 학습으로 |
| 11 | 고르기·담아 둔 학습 | 담은 개수와 다음 갈 곳, 빈 상태를 `EmptyState`로, 이용권 게이트 |
| 12 | 내 정보 | `휴대폰 번호` 값 자리에 용도가 들어 있어 번호를 어디서도 볼 수 없던 것 |
| 13 | 학부모 리포트 | 조회 중·실패에 `아직 학습 기록이 없어요`라고 영구히 단정하던 것 |
| 14 | 학원 여섯 화면 | 조회 중에 `아직 배정한 학습이 없어요` + 배정 CTA, 막힌 링크, 이름 없는 `학생` 행 |
| 15 | 운영자 목록 | 조회 중·실패에 `검색 결과가 없어요 · 검색어를 바꿔 보세요`라고 시키던 것 |
| 16 | 요금제 가드 | 조회 전 `Stepper`가 기준값으로 정책을 덮고 감사 로그를 거짓으로 만들던 것 |
| 17 | 배정 대상 판정 | 세 역할이 같은 과제에 다른 사실을 말하던 것 |
| 18 | 학부모 홈·결제 표시 | 홈이 자기 안에서 다른 말을 하던 것, 아무도 읽지 않던 `대신 내주기` 표시 |
| 19 | 학원 초대 확장 | 학부모의 자녀 연결 경로가 0개였던 것(`0036_invite_target_student.sql`) |
| 20 | 대리 보기 복귀 | 끝내면 조사하던 계정이 아니라 `/admin`으로 가던 것, 빈 유형 알림이 좁혀 보내지 않던 것 |

### 병합에서 처리해야 했던 것

**워크트리 19개가 두 커밋 뒤처진 베이스(`b729de9`)에서 갈라졌다.** D-166~169의 작업이 워커들에게
보이지 않아서, 여러 워커가 "`LoadFailed`가 없다"·"기준선은 188이 아니라 187"이라고 **정확히**
보고하고 각자 인라인 실패 패턴을 손으로 다시 만들었다. 3-way 병합에서 새 규약 쪽을 살리고
워커의 새 사실만 이식했다. 손이 필요했던 충돌은 넷이다.

| 충돌 | 어떻게 정했나 |
|---|---|
| `app/student/index.tsx` · `DESIGN.md` §14 · `student-flow` | HEAD의 세 덩어리 구조·h1 히어로 제목·`loaded` 게이트를 지키고 유닛 6의 두 사실(`byDue` 정렬 · `이어서 풀기`)만 얹었다. §14의 `담아 둔 학습이 있으면 그것부터`를 D-170으로 대체 |
| `src/features/auth/DemoAccounts.tsx` | HEAD와 유닛 1이 각자 만들었다. 유닛 1의 개선(개수 라벨·`aria-expanded`·`LiveRegion`·펼침 스크롤)에 랜딩이 쓰는 `testID`와 제어 모드(`open`·`onOpenChange`)를 더해 두 호출부가 한 컴포넌트를 쓴다 |
| `src/features/auth/AuthShell.tsx` | 유닛 1 버전이 D-166의 `headingLevel`과 톤 수정을 되돌렸다 — 다시 넣었다 |
| `e2e/a11y.spec.ts` | 유닛 1 버전이 HEAD의 `choices()` 헬퍼를 되돌렸다(풀이 화면 첫 라디오가 `5문항씩` 토글이라 순서로 세면 다른 것을 본다) — 다시 넣었다 |
| 결정 번호 | 유닛 1이 `D-166`을 자기 결정에 붙였는데 이미 쓰인 번호다. `D-171`로 재부여하고 본문 참조 13곳을 갈랐다 |

**`git stash`가 워크트리 사이에서 충돌했다.** `refs/stash`는 워크트리가 공유하므로, 기준선을 재려고
stash를 쓴 워커들이 서로의 변경을 pop해 갔다(유닛 2·4·8·11·15가 각각 사고를 보고했고 상대의 것을
패치로 보존한 뒤 자기 것을 dangling stash 커밋에서 복구했다). 최종 커밋들을 대조한 결과 각 브랜치에는
자기 파일만 들어 있다. **다음 병렬 작업에서는 워크트리에서 `git stash`를 쓰지 않는다.**

### 코디네이터가 병합 뒤 직접 고친 것

1. **`jest.config.js`·`eslint.config.js`에 `.claude/` 무시.** 워크트리 20개의 사본까지 훑어서
   테스트가 3939건(20배), 린트 오류가 41건으로 보고됐다 — 어느 체크아웃의 실패인지 구분되지 않았다.
2. **`report.ts`의 판정이 한 함수 안에서 갈려 있었다.** 유닛 17이 `pending`을 대상 행으로 좁혔는데
   같은 함수의 `academySubmit`·`byWeekday`는 반 소속으로 남아, 나중에 반에 들어온 학생의 리포트에서
   `안 낸 과제 0개`인데 `배정 4개 중 1개 냈어요`가 될 수 있었다. 셋을 같은 기준으로 맞추고
   `Deps.classIds`와 그 배관(`studentClasses`·`classKey`·`eslint-disable`)을 걷어냈다(S-013 해소).
3. **흩어진 실패 줄 17곳을 `LoadFailed` 한 벌로.** A-130이 그 컴포넌트를 만든 이유가 사본 넷이었는데,
   베이스가 뒤처진 탓에 사본이 다시 열일곱이 됐다. `again` 갈래를 더했다 — 이미 읽어 둔 값이 화면에
   있는데 재조회가 실패한 자리는 `다시 불러오지 못했어요`라고 말해야 한다(학원 화면 셋이 그 문장을
   손으로 쓰고 있었다). 전체 화면 실패(학습 상세·풀이)는 다시 시도가 화면의 주 행동이라
   `ActionBar` primary로 남겼다.
4. **`.env` 12행이 `#` 대신 `//`인 두 글자 줄이라** 셸과 Supabase CLI가 파일 전체를 파싱하지 못했다
   (`db:push`·`db:types`가 전부 막혀 있었다). 그 줄만 지웠다 — 값은 없었다.

### 되돌린 것

`npx prettier --write`를 몇 파일에 돌렸는데 **레포에 프리티어 설정이 없어서** 기본값(쌍따옴표·80칼럼)으로
파일을 통째로 재포맷했다(7파일 × 100줄 이상). 요청과 무관한 churn이라 전부 되돌리고 편집만 손으로
다시 얹었다. 이 레포는 프리티어로 포맷되어 있지 않다 — `printWidth` 90·100·110 모두 기존 파일에서
불일치를 낸다. **다음에도 이 레포에서 프리티어를 돌리지 않는다.**

### 마이그레이션

`0036_invite_target_student.sql`을 적용했다(`npm run db:push` — `0036`만 대기 중이었다).
`invites.target_student_id` 추가 · `rpc_create_invite` 4인자 재생성(마지막 인자에 기본값이 있어
기존 3인자 호출도 PostgREST가 그대로 해석한다) · `rpc_accept_invite`의 학부모 분기 · `rpc_invite_info`에
`target_student_name` · `invites_select`를 원장으로 좁힘 · `tg_invites_immutable`에 새 컬럼 추가.

**적용 후 `npm run db:types` 결과가 유닛 19가 손으로 맞춘 타입과 차이 0이었다.**
...
| `npm run lint` | 오류 0 · 경고 5(기준선 — `fonts.web.ts`의 `require` 4 + 생성 파일의 미사용 disable 1) |
| `npm test` | 198 통과 / 198(기준선 188 + 유닛 16·17의 새 테스트 10) |
| `npx supabase db push` | `0036` 적용 |
| `npm run db:types` | 손으로 맞춘 타입과 차이 0 |
| `npx playwright test --project=desktop` | **8 실패 / 174 통과.** 기준선 8건 중 7건이 그대로이고 1건은 닫혔다(`자녀가 정리한 오답노트 메모와 별표를…` — 유닛 8이 미정리 필터를 넣으면서 그 테스트의 `.last()` 전제를 함께 고쳤다). 새로 나타난 것 셋을 아래에서 다뤘다 |

### 새로 나타난 실패 셋

| 실패 | 무엇이었나 |
|---|---|
| `auth-flow` `아이디와 비밀번호로 로그인한다` | 유닛 1의 새 테스트가 `박도윤님`을 단정했다 — 화면은 `박도윤 님`이다(A-132가 띄어쓰기를 넣었고 유닛 1의 베이스에는 그 변경이 없었다). 단정을 고쳤다 |
| `parent-flow` `제출한 학생이 적으면 반 비교를 만들지 않는다` | **실제 조작 결함이었다.** 사용자 신고 ②를 고치며 넣은 펼침 스크롤을 애니메이션으로 했더니, 목록이 도착하는 동안 계속 움직여서 펼친 직후 한 줄을 누를 수 없었다. 실측(1280×800): 애니메이션이 있으면 12초 동안 `element is not stable`, 끄면 33ms. 껐다 |
| `admin-flow` `대리 중에는 오답노트 별표가 바뀌지 않는다` | 유닛 8의 다섯 개 상한 때문에 대상 문항이 접힌 쪽에 있었다. 조회를 기다린 뒤 펼치게 했다(처음엔 `isVisible()` 조건부로 넣었는데 조회 전이라 false가 돌아와 그냥 지나갔다) |

**남은 하나는 고치지 않았다** — `auth-flow` `이미 로그인한 사람에게는 내 공간으로 가는 길을 준다`가 3회 중 1회 실패한다. 원인을 측정까지 좁혔다: 로그아웃 요청은 5회 모두 204로 118~180ms에 끝나는데 화면이 5회 중 2회 `이미 로그인했어요`로 5초 이상 남는다. 네트워크가 아니라 세션 상태가 화면에 반영되지 않는 문제이고, 고치려면 `src/session/session.tsx`의 인증 이벤트 처리를 손대야 한다 — **20개 유닛 병합의 끝에 인증 경계를 끼워 넣지 않고 A-141로 남겼다.**

### 배운 것: 코디네이터가 스스로 어긴 규칙

워커 전원에게 금지한 것을 내가 어겼다. 첫 E2E 실행의 출력을 `tail`로 파이프해 중간 결과가 보이지 않았고, 끝난 줄 알고 두 번째 실행을 시작했다. **두 실행이 같은 원격 DB를 동시에 재시드해** `deadlock detected` 6건과 30초 타임아웃 4건을 만들었다 — `_fixtures.ts`가 적어 둔 바로 그 경고다. 그 실행은 버리고 단독으로 다시 돌렸다. 배경 명령의 출력을 파이프로 줄이면 진행을 볼 수 없으니, 로그는 파일로 받고 완료는 프로세스로 확인한다.


## 2026-08-19 — 오답노트를 다시 만든다: 인출 기반 복습 스케줄 (D-176)

사용자 지시는 "오답노트 세션을 전면 재기획하고 구현하라, 기획만 하고 끝내지 말라"였다. 병렬 조사
네 건(학습과학 문헌 · 국내외 서비스 설계 · 현재 화면 UX 감사 · 데이터 구조와 지난 세션 WIP 감사)을
먼저 돌렸다.

### 지난 세션의 미완성 WIP은 이어가지 않았다

잠긴 워크트리(`.claude/worktrees/agent-a5be45742a06d9213`)에 마이그레이션 3개(799줄) + `types.ts` +
`notes.ts`가 커밋되지 않은 채 있었다. 감사 결과 **이어갈 수 없었다**:

- `tsc --noEmit` 6건 실패(`db:types` 미실행). 조립 층 `reviewProvider.tsx`가 **없었다** — 세 파일이
  그 파일의 계약을 주석으로만 적어 두었고, 남은 일이 지금까지 한 일보다 컸다.
- WIP 이후 화면 셋(`result`·`notebook`·`review`)이 **+709줄 재작성**돼 그 코드가 상정한 화면이
  남아 있지 않았다.
- 🔴 `0038`이 `rpc_submit_attempt`를 `0013` 옛 본문에서 파생해 **`0029`가 실측으로 고친 NULL 가드
  두 개를 되돌렸다**.
- 🔴 소프트 삭제로 바꿨는데 `wrong_notes_write for all`을 그대로 둬서 `DELETE` 한 번으로 노트와
  복습 로그(cascade)가 사라졌다. 가드 트리거는 `before insert or update`뿐이었다.
- 🔴 `setNoteState`가 임의 상태를 써서 `stuck → queued`(처방 자력 해제)와 임의 → `collected`
  (부분 인덱스에서 빠져 큐에서 조용히 사라짐)가 가능했다.
- 🟠 누적 `passes`/`misses` 때문에 문서화된 사다리가 실제와 달랐고(`stage 3`은 도달 불가),
  졸업한 노트는 몇 번을 틀려도 정답 하나에 21일로 갔고, 오래전 네 번 틀린 노트는 한 번 틀리면
  즉시 `stuck`이 됐다.
- 🟠 뷰에 `dig is not null`을 걸어 D-054의 `여러 학생이 담은 문항 상위 N` 집계를 무력화했다.
- WIP이 확정 정책 2절을 **잘못 인용했다**: «`이해 완료 표시는 열지 않는다`»라는 문장은 없다.
  실제는 «별표와 이해 완료 표시는 열지 않는다»이고 **학원 공개 여부만** 정한다 — 학생 화면에서의
  존속을 정하지 않는다.

**살린 것**: `note_reviews` 표라는 발상, `(note_id, reviewed_on)` 유니크로 "서로 다른 세션"을
날짜로 정의한 것, 파생값을 누적 컬럼으로 두지 않는 규율, 가드 트리거로 스케줄을 클라이언트에서
떼어낸 구조. **나머지는 다시 썼다.**

### 조사가 한 곳을 가리켰다

학습과학은 "자기 신고를 숙달 판정에 쓰지 말고 인출 결과로 대체하라"고 했고(학생의 자기 예측은
실제 성과와 **무상관**), 코드 감사는 "그 자기 신고(`mastered`)가 실제로 아무것도 하지 않는다"고
했다. 경쟁 조사는 **자동 수집은 이미 해결된 문제이고 진짜 지점은 재방문**이라고 했다 — 시장
리더 콴다가 자동 수집형 오답노트를 독립 화면에서 내렸다.

그리고 두 조사가 겉보기에 충돌했다: 학습과학은 "다시 풀기가 효과의 본체(g=0.50)"라고 하는데
경쟁 조사는 "두 번째 시도에서 무너진다"는 실증을 가져왔다(`답을 그냥 기억해서 20분에 끝났다`).
**국어는 더 심하다 — 지문까지 기억에 남는다.** 조합해서 풀었다: 같은 문항을 다시 풀되 답을 고르는
것으로 끝내지 않고 **근거를 지목하게** 하고, **선지 순서를 섞는다.**

### 만든 것

- **마이그레이션 셋**(`0037`~`0039`): `note_reviews` 표 · `wrong_notes`에 스케줄 5컬럼 ·
  `note_state`(`queued`·`graduated`·`stuck`) · `note_evidence`(`passage`·`choices`·`unsure`) ·
  가드 트리거 · `wrong_notes` DELETE 권한 회수 · 학원 뷰에 `dismissed_at is null` ·
  `review_done` 이벤트를 `mastered`에서 복습 로그 INSERT로 이전 · RPC 넷
  (`rpc_add_wrong_note` · `rpc_log_note_review` · `rpc_requeue_note` · `rpc_set_note_review_recap`).
- **스케줄은 두 카운터만**(`streak`·`miss_streak`). `stage`와 누적 `passes`를 버려서 WIP 버그 셋이
  구조적으로 발생할 수 없게 했다.
- **순수 로직을 모듈로 분리**(`src/features/review.ts`) — 큐·상한·우선순위·문장·셔플. 단위 테스트
  35건이 여기 붙는다.
- **화면 다섯**: 카드 복습 전면 재작성 · 오답노트(다음 차례 · 멈춘 문항 복귀 · 복습으로 가는 길 둘 ·
  대리 보기 메모 고지) · 학습 탭(`오늘 볼 오답` 첫 줄 + 왜 지금인지 말하는 캡션) ·
  홈(`확인할 것` 줄) · 결과(`틀린 문항 모두 담기`).

### 사용자에게 보이는 규칙

- 복습 한 장: `문항(지문 접힘) → 답 → **근거** → 확인 → 정답·처음 답·해설·**다음 차례** →
  **내 말로 한 줄**(선택) → 질문 → 다음`. 확인 전에는 정답이 없다.
- 차례: `오답 1일` · `정답 7일` · `21일` · `연속 3회면 익힘(30일 유지)` · `연속 오답 3회면 쉼`.
- **하루 5개까지.** 밀려도 크기를 늘리지 않고 우선순위만 바꾼다(다시 틀린 것 → 아직 못 익힌 것 →
  오래 밀린 것). 문구는 `37개 밀렸어요`가 아니라 `오늘은 3개만 보면 돼요`.
- 단계 숫자·강등·스트릭·포인트를 두지 않는다. 대신 `3번 다시 풀었어요`를 말한다.
- **담은 날에는 차례가 없다** — 첫 차례는 내일이다. 화면이 그 사실을 말한다
  (`방금 담은 오답은 내일 다시 볼 차례예요.`).

### 근거가 약한 선택을 밝힌다

1. **근거 3택**은 선지별 확신 배분(d=0.51)보다 근거가 약하다. 5지선다마다 배분 UI를 반복하는
   모바일 부담 때문에 골랐고, 확실한 것은 **단순 확신도 평정은 이득이 없다**(32.1% vs 32.7%, n.s.)는
   쪽이다.
2. **선지 섞기**는 효과크기 근거가 없다(문항 형식 변환의 보존 효과는 d=0.07). 답 위치 기억을 막는
   것이 목적이고 그 관찰은 사용자 서술에서 왔다.
3. **하루 5개**는 근거로 고정된 숫자가 아니다. "밀린 큐 → 이탈" 인과 자체는 동료심사 근거가 없고,
   큐를 없앤 서비스가 이겼다는 사실과 커뮤니티 관찰이 근거다.
...
|---|---|
| `npm run typecheck` | 오류 0 |
| `npm run lint` | 오류 0 · 경고 5(기준선 — `fonts.web.ts`의 `require` 4 + 생성 파일 1) |
| `npm test` | **228 통과 / 228**(기준선 198 + `review.test.ts` 30) |
| `npx tsx scripts/verify-note-schedule.ts` | **54 단정 통과 / 실패 0** |
| `npm run db:push` · `db:types` | `0037`~`0039` 적용 · 생성 타입과 코드 일치 |
| `npx tsx scripts/gen-seed.ts` + `run-sql.ts` | 시드가 스케줄 컬럼을 직접 쓴다(아래) |

**`scripts/verify-note-schedule.ts`를 새로 만들었다.** 이 상태 기계는 SQL 안에 있어서 단위
테스트로 닿지 않는다. seed 계정으로 **실제 로그인해서** 진짜 JWT로 RPC를 부르고, 한 걸음마다
소유자 연결로 로그 날짜를 하루 물려 사다리를 걷는다. 확인하는 것: 정답 사다리 7 → 21 → 졸업(30) ·
같은 날 두 번째 복습 거부 · 졸업 뒤 오답 1회는 `stuck`이 아니다 · 연속 오답 3회 → `stuck` ·
큐 복귀와 다른 상태에서의 거부 · `state`·`due_on`·`streak` 직접 UPDATE 거부(메모는 통과) ·
`note_reviews` 직접 INSERT 거부 · `wrong_notes` 물리 DELETE 거부 · 남의 노트·지운 노트 거부 ·
되살리기가 유니크 위반 없이 같은 행을 살리고 스케줄을 보존 · 학원 뷰에 새 컬럼 8종 전부 없음 ·
선생님은 `note_reviews`에서 0행. **끝나면 만든 것을 전부 치우고 시작 상태를 복원한다.**

**시작 상태를 단정하지 않게 고쳤다.** 처음에는 백필 결과를 단정했는데, seed가 화면 확인용으로
졸업·멈춤 상태를 섞어 넣으므로 어느 노트가 뽑히느냐에 따라 값이 달라졌다. 이 스크립트가 검증할
것은 **전이**이지 seed 내용이 아니다 — 소유자 권한으로 시작점을 맞춘 뒤 걷는다.

### 시드가 스케줄을 직접 쓴다

가드 트리거는 INSERT의 스케줄 값을 무조건 `queued · 내일`로 덮는다. 그대로 두면 **시드 직후 오늘
볼 오답이 0개**가 되어 학생 홈·학습 탭·카드 복습을 화면에서 확인할 수 없다. 그래서 `gen-seed.ts`가
서버 플래그를 세우고 값을 직접 넣고 끝에서 되돌린다. **다시 볼 날은 `day()`로 옮기지 않는다** —
그 함수는 `ANCHOR`로부터의 간격을 계산하는데 스케줄은 실제 오늘이 기준이라 두 기준이 섞이면 며칠씩
어긋난다.

시드에 세 상태를 섞었다: 오늘 차례 6개 · 익힘 1개(+22일) · 쉬는 것 1개(연속 오답 3).

### E2E

새로 12건을 만들었다. **이 흐름에는 회귀를 잡을 장치가 하나도 없었다** — `review-master`·
`review-restart`·`review-load-failed`·`note-filter-starred`, 복습 중 이탈 후 재진입에 대한 단정이
0건이었다(A-084가 기록한 공백의 일부).

첫 전체 실행에서 21건이 실패했다(기준선 8건 + 새로 13건). 원인을 넷으로 갈랐다:

| 무엇 | 원인 | 어떻게 |
|---|---|---|
| 새 테스트 10건이 정확히 30초 타임아웃 | `page.goto('/login')` 없이 `loginHere`를 불렀다 | `loginAs`로 바꿨다 |
| `학습 탭의 오답노트 → 카드 복습` | **실제 결함이었다.** 방금 담은 오답은 차례가 내일인데 학습 탭 문구가 `차례가 된 오답이 없어요`라고만 했다 — 설계는 맞고 문구가 틀렸다 | `soonestDueDays`를 만들어 `방금 담은 오답은 내일 다시 볼 차례예요.`로 갈랐다 |
| `오답노트는 대화가 입력창 위에…` | **실제 회귀였다.** `keepWrongNotes`가 연속으로 눌렀는데, `rpc_add_wrong_note`는 멱등이라(지운 것을 되살리는 길이다) 상태 반영 전 두 번째 클릭이 같은 줄을 다시 담아 조용히 삼켜졌다. 사용자에게는 오히려 나은 동작이다 — 전에는 유니크 오류가 났다 | 헬퍼가 담긴 개수를 확인한 뒤 다음을 누른다(기다림이 곧 단정이다) |
| 진행 영속·별표 덱·완료 요약·일괄 담기 | 테스트가 새 동작을 잘못 전제했다: 하루 상한 때문에 총계 산술이 성립하지 않고(차례 6개 → 덱 5장, 한 장을 끝내도 남은 다섯이 그대로 다섯), 확인한 카드는 오늘 몫을 다 해서 어느 덱에서도 다시 나오지 않고, `count()`는 기다리지 않아 조회 전에 루프를 빠져나왔다 | 같은 카드가 다시 오지 않는 것으로 단정을 바꾸고, 별표는 확인하지 않은 카드에 달고, 덱이 그려지기를 기다린다 |

고친 뒤 오답 복습 관련 12건 + 갱신한 2건이 **14/14 통과**했다.

### 배운 것

**`nohup ... &`를 배경 실행 도구 안에서 다시 쓰지 않는다.** 셸이 먼저 끝나 도구가 완료로 보고했고,
실제 Playwright는 계속 돌고 있었다. 그 뒤 래퍼가 타임아웃될 때 프로세스 그룹이 함께 죽어 실행이
143/192에서 잘렸다. macOS에는 `setsid`도 없다 — 도구의 배경 실행을 그대로 쓴다.

## 2026-08-19 — 여섯 관점 검토가 찾은 것을 고친다 (D-177)

D-176을 구현한 뒤 사용자 지시대로 여섯 관점을 병렬로 투입했다: 교육적 효과 · 수험생 UX ·
반복 사용성 · 모바일 사용성 · 디자인 완성도·일관성 · 정확성·보안. **읽기 전용 검토자 여섯이 모두
실재하는 결함을 찾았고, 그중 넷은 이 설계의 목적을 무너뜨리는 것이었다.**

### 목적을 무너뜨린 넷

**① 정오가 클라이언트 인자였다.** `rpc_log_note_review`가 `p_is_correct`를 받아 그대로 스케줄에
썼고, 서버는 `questions.answer_index`와 **대조하지 않았다.** 학생 JWT로

    POST /rest/v1/rpc/rpc_log_note_review  {"p_note_id": "<내 노트>", "p_is_correct": true}

를 서로 다른 3일에 부르면 문항을 열어 보지도 않고 `graduated`가 됐다. `0037`의 주석이 적어 둔
목적 — "학생이 이 표에 직접 넣을 수 있으면 정답 3회를 손으로 만들어 졸업할 수 있고, 그러면 이 표가
학습의 사실이 아니라 자기 신고가 된다 — `mastered`가 정확히 그랬다" — 을 그대로 어겼다. **표에
직접 넣는 길만 막고 정오 판정은 클라이언트에 맡긴 것이다.** 같은 레포의 `rpc_submit_attempt`는
이미 서버 채점을 하고 있었고(`coalesce(x.picked_index = q.answer_index, false)`) 채점 재료도 함수
안에 있었다. 인자에서 정오를 없애고 고른 자리만 받는다.

**② 범위 덱의 복습이 스케줄을 전진시켰다.** `?all=1`·`?starred=1`·`?area=`는 차례를 보지 않고
열리는데(시험 직전에 범위를 넓게 돌아보는 것은 스케줄과 다른 목적이다) `rpc_log_note_review`가
차례를 검사하지 않았다. 그래서 **3일 연속 전체 복습으로 `1 → 7 → 21`을 3일로 압축해 전부
졸업**시킬 수 있었다 — `0037`의 유니크 키가 날짜로 막으려던 바로 그 상태다. 차례가 아니면 기록만
남기고 일정은 그대로 둔다(`scheduled: false`).

**③ `wrong_notes`의 INSERT·UPDATE가 열려 있었다.** `0037`은 DELETE만 회수했다. `wrong_notes_write`가
`for all using (student_id = auth.uid())`이므로 `student_id`만 자기 것이면 나머지 컬럼은 자유였다 —
학생이 **임의 문항 + 임의 `dig` 본문 + 우리 학원 배정 uuid**로 행을 만들거나 기존 행을 그렇게
PATCH해, `v_academy_visible_notes`를 통해 **담당 선생님이 읽는 화면에 주입**할 수 있었다.
0026 §2가 뷰에서 한 번 고친 것과 같은 모양이다. INSERT를 회수해 RPC를 유일한 담기 문으로 만들고,
가드가 `student_id`·`question_id`·`content_set_id`·`source`·`assignment_id`를 얼린다.

**④ 건너뛰기가 큐를 교착시켰다.** 기록을 남기지 않으므로 `due_on`이 과거에 남고, 다음 날 밀린
일수가 1 늘어 우선순위에서 **더 앞으로** 갔다. 모르는 카드 다섯 장을 건너뛰면 다음 날 덱이 같은
다섯 장이고 그 상태가 영구히 유지됐다 — `miss_streak`도 늘지 않아 `stuck` 탈출구조차 열리지
않았다. `rpc_defer_note`가 서버 경로로 하루 미룬다(기록은 여전히 남기지 않는다).

### 계산으로 드러난 것

교육적 효과 검토가 코드 규칙을 그대로 시뮬레이션했다.

- **하루 5개 상한이 사다리를 무력화한다.** 밀린 일수 내림차순 정렬이라 첫 바퀴 백로그가 예정된
  2·3회차보다 항상 앞선다. 노트가 `5 × 7 = 35`개를 넘으면 **7일 칸은 도달 불가**이고 간격은
  사다리가 아니라 `노트 수 ÷ 5`가 정한다. 오답 100개면 첫 한 바퀴만 20일, 전원 익힘까지 61일.
- **정답률 중간대 문항이 영원히 순환한다.** 오답↔정답을 번갈아 맞히면 `miss_streak`이 매번 0으로
  초기화돼 절대 `stuck`이 안 되고 `streak`도 2를 못 넘어 절대 졸업하지 않는다. 정답률 80%·100개
  시뮬레이션에서 상한 5칸 중 평균 19%가 이 카드로 채워진다.
- **`설명 피드백 0.49`를 인용했는데 데이터가 그 층이 아니다.** 시드 169문항 파싱 결과 **오답 선지를
  언급한 해설 0건(0%)**, 정답 재진술 22건, 평균 21.7자. 인용한 세 층 중 가장 높은 층이 화면에도
  데이터에도 없다.
- **반복 사용성**: 오답 10개를 담은 학생은 8일 중 **6일이 `0개`**다(담은 날 + 8/22~8/26). 그리고
  **알림 채널이 코드에 하나도 없다**(`expo-notifications`·웹푸시·배지 전부 0건) — 서버가 날짜를
  정하는 스케줄러의 가치는 "그 날짜에 학생이 온다"는 전제에서 나오는데, 그러면 이 설계는 재방문을
  **만드는** 장치가 아니라 이미 있는 재방문을 **배분하는** 장치다.

앞의 둘과 해설 데이터는 이번에 구조를 바꾸지 않았다 — 상한·간격·콘텐츠 작업은 각각 별개 결정이다.
아래 `남기는 쟁점`에 적었다.

### 내가 만든 새 거짓말

디자인 검토가 찾았다. D-176이 대리 보기의 `8개 중 8개는 아직 정리하지 않았어요`(거짓)를 고쳤는데,
**거짓의 방향만 뒤집혔다.** `maskDig`가 대리 보기에서 **모든** 노트에 `digHidden`을 붙이므로
`needsMemo`가 전부 거짓이 되고 미정리가 0 — 하나도 정리하지 않은 학생의 오답노트가
`모두 정리했어요`가 됐다. 지금은 **세지 않고 셀 수 없다고 말한다.**

### 화면 쪽으로 고친 것

| 무엇 | 왜 |
|---|---|
| 카드가 바뀌면 카드 상태를 무효로 읽는다 | 다른 기기에서 노트를 지우면 `at`이 조용히 다음 자리로 올라가는데 `resetCard`는 불리지 않았다 → 기록 없이 정답·해설 공개, 누른 적 없는 선지에 `내 답`, 앞 카드 대화가 **다른 노트의 메모로 저장**. D-113이 막았다고 적은 어긋남의 재발 |
| 세션 날짜를 고정 | `todayISO()`가 렌더마다 새 값이라 자정에 선지 순열이 바뀌어 **다른 선지로 채점**됐다 |
| `다음 문제`를 질문 섹션 **위**로 | A-109를 `Section` 밖으로만 뺐고 문서 순서는 그대로였다 — `ActionBar`는 고정 바가 아니므로 두 입력창 아래였다. 확인 후 문서가 약 1800px(웹 3.1화면)이고 `다음 문제`가 마지막 44px |
| 지문 자동 펼침 제거 | 확인 직후 지문이 **위에서** 약 400px 자라 정답·해설이 화면 밖으로 밀렸다. 커밋 `cb588f0`이 고친 것과 같은 계열 |
| 선지 선택에 동그라미 + `accentSoft` | 테두리 색 하나로는 §11 위반이고, 같은 문항을 처음 푸는 화면(`solve`)과 모양이 달랐다 |
| 근거 3택을 선지와 다른 형태로 + `radiogroup` | 같은 모양의 라디오 두 벌이 서서 어느 것이 답이고 어느 것이 근거인지 갈리지 않았다 |
| 지문 없는 문항의 근거 문구를 갈라 씀 | 문법 세트(34문항)에 `지문에서 근거를 찾았어요`가 떴다 — 뜻이 서지 않는 3택에서 아무거나 골라야 확인 버튼이 나타났고, 문법은 오답이 가장 많이 쌓이는 영역이다 |
| 건너뛰기 44px | 카드마다 반복되는 유일한 출구가 32px이었다. 8px 위가 4번째 선지다 |
| `Steps`는 끝낸 것만 칠한다 | `seen + 1`이라 마지막 카드에서 답하기 전에 칸이 전부 찼다 — 끝난 줄 알고 나간다 |
| 정답 표식을 선지 아래 줄로 | 같은 줄에 두면 390에서 본문 폭이 276 → **162px**(10.8자/줄)로 깎여, 확인 후 가장 오래 읽는 문장이 4줄이 됐다 |
| `정답 · …` 캡션 제거 → `이번 답` / `처음 답` | 정답을 두 번 말하고, 이번 답과 처음 답은 다른 형식이라 나란히 비교할 수 없었다 |
| 완료 요약의 `쉬는` 문항 분기 | 세 번째로 틀린 문항에 `내일 다시 만나요`라고 단정했다 — 서버는 그 카드를 큐에서 내린다(`due_on = null`). 한 흐름의 두 화면이 같은 카드의 일정을 반대로 말했다 |
| 완료 요약의 모순 제거 | `차례가 된 오답을 모두 봤어요` 바로 아래에 `남은 오답 더 보기 · 5개가 더 있어요` |
| 학습 탭이 한 화면에서 40과 5를 말하던 것 | 캡션이 상한 미적용 원값이었다. 개수를 세는 곳을 `todayCount` 하나로 |
| 줄에 적힌 개수 = 덱 개수 | `오답 60개`를 누른 학생이 `모두 55개`를 받았다 |
| 홈 줄 44px + 이동 표시 | 캡션 한 줄(20px)이 `Pressable`인데 `tap.textLine`이 없었고, 위아래 두 줄과 모양이 같아 눌린다는 신호가 없었다 |
| 한 줄 정리: 저장 뒤 되보여 주고 넘길 때 버리지 않는다 | 쓰기 전용 서랍이었다. `한 줄은 선택이다`는 쓰지 않아도 된다는 뜻이고 쓴 것을 버려도 된다는 뜻이 아니다 |
| `AskField`에 `sendLabel` | 보내기 이름이 `물어보기` 고정이라, 한 줄을 저장하려는 스크린리더 사용자가 `물어보기`를 눌렀다 |
| 재조회 실패가 세션을 날리던 것 | 별표 실패 → `reload()` → 그 재조회 실패 → 화면 전체 교체 → 덱·고른 답·대화·쓰던 한 줄 소실. `loaded` 게이트(D-163)와 같은 사고 |
| `Screen`에 키보드 prop | `keyboardShouldPersistTaps` 기본값이 `'never'`라 키보드가 떠 있으면 저장 버튼 **첫 탭이 삼켜진다** |
| `clock.ts`의 무효한 가드 | `NaN ?? 1`은 `NaN`이다 — 가드가 있는 것처럼 보이는 것이 없는 것보다 나쁘다 |
| 오답노트에 `쉬는 중` 칸 · 한 줄 표시 | 멈춘 노트를 찾는 유일한 방법이 목록을 전부 펼쳐 훑는 것이었다(돌아오는 문이 그 카드 안에만 있는데). `recap`은 저장만 되고 읽는 화면이 0곳이었다 |
| 시드에 복습 기록 6행 | `state: 'graduated', streak: 3`이 "서로 다른 날 세 번 맞혔다"는 주장인데 근거 행이 0개였다 — 화면의 `지금까지 N번 다시 풀었어요`가 어떤 카드에도 뜨지 않았다. `review_done` 이벤트도 `mastered`에서 만들던 것을 복습 기록에서 파생시켰다 |
| `note_reviews`의 익명 SELECT 회수 | 0035가 세운 규칙(Supabase 기본 부여). 정책이 이미 0행을 내지만 벽을 둘로 만든다 |
...
| `npm run lint` | 오류 0 · 경고 5(기준선) |
| `npm test` | **237 통과 / 237** |
| `npm run db:verify` | **176 통과 / 0 실패**(+10 — 복습 기록의 본인·자녀·운영자·익명·교직원 다섯 방향) |
| `npm run db:verify:schedule` | **84 통과 / 0 실패**(+30) |
| `npx playwright test --project=desktop` | 아래 |

**`db:verify:schedule`을 `package.json`에 노출했다** — 54단정이 루틴에 붙지 않고 `npx tsx …`로만
돌 수 있었다.

검증 스크립트에 더한 것: 서버 채점(오답을 보내면 오답으로 판정 · 옛 시그니처가 사라졌는지 ·
선지 범위 · 음수 · 인자 누락) · 차례가 아닌 복습(기록은 남고 일정은 그대로) · 찍어서 맞힘 ·
멈춘 문항 거부 · 건너뛰기(미룸 · 기록 없음 · 차례 아닌 것은 미뤄지지 않음 · 남의 노트) ·
한 줄 RPC 전체(0039에 대한 단정이 **0개**였다) · 정체성 컬럼 다섯 · `wrong_notes` 직접 INSERT.

**`backdate` 헬퍼를 고쳐야 했다.** 로그만 하루 물리면 `due_on`이 그대로라 새 규칙(차례가 아닌
복습은 일정을 안 움직인다) 때문에 사다리가 첫 칸에서 멈춘다. 실제로는 7일·21일이 지나야 그날이
오지만 테스트에서 시간은 흐르지 않으므로 "그날이 왔다"를 소유자 권한으로 함께 재현한다.

### 남기는 쟁점 (이번에 고치지 않았다)

1. **하루 5개 상한이 35개 이상 학생에게 사다리를 무력화한다.** 상한을 올리거나 졸업 조건을 낮추는
   일이고, D-176의 근거 문단 전체를 다시 여는 결정이다.
2. **정답률 중간대 문항의 무한 순환.** `miss_streak` 대신 창(window) 판정이 필요하다 —
   `note_reviews`에 전체 이력이 있어 새 컬럼 없이 가능하다.
3. **해설이 설명 피드백 층이 아니다**(오답 선지 언급 0/169). `Question`에 선지별 사유 필드를 두고
   콘텐츠를 채우는 일이라 스키마 + 콘텐츠 작업이다.
4. **알림 채널이 없다.** 마스터 플랜 2절이 전화번호 용도에 `알림`을 이미 적어 두었으므로 정책
   개정은 필요 없고, 발송 인프라·수신 범위·시각 결정이 필요하다.
5. **8일 중 6일이 `0개`인 구간에 홈이 침묵한다.** 차례가 없는 날 복습을 홈에 두면 D-176이
   D-132를 개정한 근거(`차례가 오늘이면 오늘 할 일이다`)가 깨진다.
### 사다리 근거 문장을 정정했다

`0038`의 주석은 "7일이 중심이다"라며 확장 vs 균등이 유의하지 않다는 것과 교실에서 7일 고정이 가장
일관되게 양의 효과를 냈다는 것을 근거로 들었다. **그 두 인용은 균등 간격을 지지하는데 구현은
`1 → 7 → 21 → 30` 확장 곡선이다.** 인용이 붙은 숫자는 7일 하나이고 `21`·`30`은 근거가 없다.

정확히 말하면: 차이가 **유의하지 않으므로** 확장을 고르는 것이 문헌에 반하지는 않는다. 그러나
문헌이 확장을 **지지하지도** 않으므로 이 사다리는 근거로 고정된 것이 아니라 **선택**이다.
`stuck`을 3회로 좁힌 근거도 틀렸다 — `0038`은 "Anki의 8회는 몇 달이 걸린다"고 적었지만 오답은
항상 다음 날이므로 **8회면 8일**이다. 3 역시 선택이다.

적용된 마이그레이션 파일은 고치지 않는다(체크섬·이력). 정정은 `src/features/review.ts`의
`GRADUATE_STREAK` 주석에 적었다 — 화면과 서버가 그 숫자를 공유하는 자리이고, 다음 사람이 사다리를
만질 때 먼저 읽는 곳이다.

## 2026-08-19 · 오답노트 재구성 뒤 정리(`/simplify` 네 관점)

디프(마이그레이션 5개 · 화면 6개 · 기능 4개 · 스크립트 3개 · E2E 3개)를 재사용 · 단순화 · 효율 ·
고도 네 관점으로 훑고 **품질만** 고쳤다. 동작을 바꾸는 항목은 넘기지 않았다.

### 고친 것

1. **복습 화면의 카드 상태 열 개를 하나로.** `useState` 열 개가 카드 하나의 진행을 나눠 들고
   있어서 카드가 바뀔 때 열 번 초기화했고, 그중 하나를 잊으면 **앞 카드의 값이 다음 카드에
   보였다.** `CardState` 하나에 담고 `id`로 소유를 확인한다(`cs = cardState?.id === card.id ? … : null`).
   `step`도 상태가 아니라 파생값이다(`checked → done`, `slot == null → pick`, …).
2. **`closingLine`을 화면에서 `review.ts`로.** 완료 문장 네 갈래는 순수 함수이고, 화면에 두면
   테스트가 렌더를 거쳐야 한다.
3. **`scripts/_verify.ts`를 만들었다.** `check`·`eq`·`signIn`·`ownerClient`·`requireEnv`·`results`가
   두 검증 스크립트에 복제돼 있었다. 풀러 주소에 `connectionString`과 `password`를 함께 주면 안
   되는 이유(실측)도 한 자리에만 남는다. **다만 `verify-note-schedule.ts`만 옮겼다** — 그래서
   사본은 아직 줄지 않았다(아래 넘긴 것 참고).
4. **가드를 여닫는 세 줄이 아홉 자리에 있던 것을 `asOwner()`로.** `try/finally`가 없어서 중간
   쿼리가 던지면 `scody.note_schedule`이 이 연결에 켜진 채 남았다. 새는 범위는 **소유자 쓰기**다
   — 가드 거부 단정은 전부 `student`(PostgREST)로 보내는 다른 연결이라 이 GUC가 닿지 않는다.
5. **`GRADUATE_STREAK`을 검증 스크립트가 읽는다.** 졸업 기준이 TS와 SQL 양쪽에 적혀 있어 한쪽만
   바뀌면 조용히 어긋난다. 이제 그 단정에서 걸린다(통과 85 · 실패 0).
6. **E2E 헬퍼 둘.** `openTodayDeck`(열 자리 전부) · `openNotebookAll`(student-flow 네 자리).
   `page.goto`로 줄이지 않았다 — 하드 로드는 대리 보기 세션을 끊는다. `openNotebookAll`은
   `admin-flow.spec.ts`의 두 자리를 아직 안 지난다(그쪽은 `expect(more).toBeVisible()`로 더 세게
   기다린다 — 헬퍼도 조회 종료를 먼저 기다리게 고쳤다).
7. **조용히 통과하던 단정 둘을 못 박았다.** `범위 덱의 복습은 다음 차례를 앞당기지 않는다`가
   `if (found)`로 감싸여 있어 시드가 바뀌면 아무것도 보지 않고 통과했다. 완료 요약의 헷갈린 줄
   개수는 `풀었고 N개를 맞혔어요` 문장과 묶어서 센다(선지를 섞어 전부 정답이 되는 날에도 확인된다).
8. **`0042_note_review_comments.sql`.** 0037~0040이 이유를 마이그레이션 파일에만 적어서
   `\df+ rpc_log_note_review`에는 계약이 없었다 — 0026·0032·0033·0036의 관례대로 함수 6개 ·
   `note_reviews` · 스케줄 컬럼 4개에 `comment on`을 붙였다. 코드 변경 없음.
9. **`gen-seed.ts`의 날짜 산술을 앱과 같은 함수로.** `daysBetweenISO(todayISO(), …)`로 바꾸고,
   `(current_date ± N)::date`를 만드는 세 줄이 세 곳에 복제된 것을 `dateFromToday()`로 모았다.
   **생성 결과는 바이트 단위로 같다**(`git diff` 0줄). 앞 판본에 시간대 결함이 있었던 것은
   아니다 — 두 값을 모두 UTC 자정으로 맞춰 뺐으므로 결과가 같았고, 고친 이유는 사본 수다.

### 검토에서 고친 것 (같은 날, `/code-review`)

위 아홉 개를 다시 훑어 나온 것 중 동작 결함을 고쳤다.

1. **`틀린 문항 모두 담기`가 실패를 성공으로 말했다.** `addWrongNotes`의 반환형에 `added`·`failed`를
   **필수**로 적고 `as`로 덮어서, `write`가 예외를 잡은 갈래(`{ ok: false, error }`)가 그 형태를
   통과했다. 그러면 호출부의 `failed > 0`이 `undefined > 0`이 되어 `undefined개를 오답노트에
   담았어요`가 성공 토스트로 나간다. 두 필드를 선택으로 바꾸고(위 `restored?`와 같은 형태) 캐스트를
   지웠다. `Promise.all` → `allSettled`도 함께 — 첫 거절에서 끝나면 이미 커밋된 나머지를 세지 못한다.
2. **복습 기록 게이트가 운영자를 지나쳤다.** 조건이 `isAcademy && …`라 `roles: ['admin']`에는 걸리지
   않았고, 주석이 말한 노출(남의 `recap`이 세션에 올라오는 것)이 그대로였다. 소비처가 있는 역할로
   조건을 다시 적었다 — `student`·`parent`가 아니면 부르지 않는다.
3. **한 줄 저장이 다음 카드의 답을 지웠다.** `patchCard`는 없는 상태를 만들어 주므로 `await` 뒤에
   도착한 갱신이 **앞 카드 id로 자리를 차지한다**. 저장 → 다음 문제 → 새 카드에서 답 고르기 순으로
   누르면 새 카드의 `cs`가 `null`이 되어 고른 답·근거와 확인 버튼이 사라졌다. `patchLive`(카드가
   바뀌었으면 버린다)를 두고 `await` 뒤 네 자리를 그쪽으로 옮겼다. `convo`도 `prev` 위에 얹는다.
4. **완료 요약이 내일 오지 않는 카드에 `내일 다시 만나요`라고 말했다.** `resting`을 `state ===
   'stuck'`으로 세서 **차례가 아니었던 복습**(`scheduled: false` — 범위 덱의 회차는 `due_on`을
   움직이지 않는다)이 빠졌다. 같은 흐름에서 카드 화면은 `다음 차례는 그대로예요`라고 말하고 요약은
   `내일`이라고 말했다. `notTomorrow`로 이름을 바꾸고 상태가 아니라 **날짜**로 센다.
5. **`익힘 도달` 문장이 30일 유지 복습마다 되풀이됐다.** `state === 'graduated'`는 유지 복습에서도
   참이고, 그 줄이 if/else의 앞이라 `review-next-due`가 유지 복습에서 한 번도 그려지지 않았다.
   서버가 `streak`을 돌려주므로 `streak === GRADUATE_STREAK`으로 바꿨다.
6. **범위 덱의 빈 화면이 노트 0개 계정을 빈 오답노트로 보냈다.** 표의 `to`만 보면 별표·전체·영역이
   항상 오답노트를 가리켜서, 표 주석이 정한 규칙(`담은 오답이 아예 없는 계정만 학습 탭으로`)이
   행동 축에서 깨졌다. 직접 URL로 닿는 자리다. 조건을 한 번 물어 세 곳에 쓴다.
7. **E2E 둘.** `expect(found).toBe(true)`에 덱 렌더 대기를 붙였다 — 제목은 조회 중에도 그려지므로
   그 단정만으로는 `count()`가 0을 보고 빠져나와 스케줄과 무관하게 실패한다(형제 테스트가 같은
   대기를 이미 갖고 있다). 헷갈린 줄 개수는 ①정규식 실패를 먼저 못 박고 ②펼친 뒤에 세고
   ③`review-missed-more`를 접두어로 함께 잡지 않게 고쳤다.
8. **`0042`의 `note_reviews` 읽기 범위가 틀렸다.** `학생 본인만 읽는다(선생님·학부모는 0행)`이라고
   적었는데 정책은 `can_read_student(student_id) or is_admin()`이다 — 연결된 학부모와 운영자가
   읽는다. `\d+`로 계약을 읽게 하려고 만든 파일이라 그 자리의 오기가 없는 것보다 나쁘다.
9. **죽은 것.** `dueCards`의 `reviews` 인자(`reviewCount`를 지우면서 읽는 곳이 없어졌다),
   `_verify.ts`의 `count`(부르는 곳이 없고 `verify-rls.ts`에 사본이 그대로 있다),
   `NoteStateCounts` 위에 겹친 주석 블록(`stateCounts`가 문서 없이 남았다),
   `learn.tsx`가 가리키던 `counts.today`. `EVIDENCE_ORDER` 순서 단정을 하나 더했다 —
   `Object.keys` 순서라 라벨 맵을 재배열하면 화면이 조용히 바뀌는데 `as` 캐스트가 타입 검사도 막는다.
...
| 린트 | `npm run lint` | 오류 0 · 경고 5(기존: `router.d.ts` 1 · `fonts.web.ts` 4) |
| 단위·통합 | `npm test` | **241 통과 / 241**(13 스위트). 237 → 240(`closingLine` 3건) → 241(`EVIDENCE_ORDER` 1건) |

`npm run test:e2e`와 `scripts/verify-note-schedule.ts`는 돌리지 않았다 — 각각 개발 서버와 원격 DB
자격 증명이 필요하다. 위 7번의 E2E 변경과 5번의 `review-next-due` 갈래는 **정적 근거만 있다.**

### 넘긴 것과 이유

- `RadioMark`·`StatusDot`·`AiThread`·`useNoteAsk` 추출, 오답노트의 `counting` 재구성 —
  범위 밖 파일을 함께 바꾸거나 공용 컴포넌트를 새로 만드는 일이다. 각각 독립이라 나중에 하나씩.
- `verify-ai.ts`·`run-sql.ts`를 `_verify.ts`로 옮기기 — 이번 디프가 건드리지 않은 스크립트다.
- **`verify-rls.ts`를 `_verify.ts`로 옮기기** — 1300줄 · 단정 125개이고 돌려 볼 수 없다(원격 DB).
  그래서 사본이 아직 세 곳이다(`_verify.ts`·`verify-rls.ts`·`verify-ai.ts`). 옮길 때까지 `_verify.ts`를
  고치면 `verify-rls.ts`도 함께 본다.
- `app/student/index.tsx:331`의 `useMemo(() => todayCount(wrongNotes, todayISO()), [wrongNotes])` —
  `learn.tsx`에서 고친 것과 같은 결함이다(의존성에 날짜가 없어 자정을 넘긴 세션이 어제 수를 말한다).
  이번 범위 밖 파일이라 열어 두고 A-146으로 적었다.
- **하루 상한(5)과 헷갈린 줄 상한(`MISSED_PREVIEW` 5)이 같은 숫자에 기대는 것** — E2E는 펼친 뒤에
  세게 고쳤지만 두 상수는 여전히 서로 다른 근거로 정해져 있다. 묶을지는 결정 대기다.
- 완료 요약 테스트를 둘로 쪼개기 — 쪼개도 12회 덱 순회를 두 번 하게 될 뿐이고, 선지를 섞으므로
  두 번째 테스트도 결정적이 되지 않는다. 대신 개수 단정을 요약 문장에 묶었다(위 7번).
- `acceptsReview` 술어, SQL `write_note_schedule` 헬퍼 — 적용된 마이그레이션은 고치지 않는다.

### 문서 정정

- `M-DEC-4`의 "새 컬럼 없이 가능"은 **틀렸다.** 0040부터 차례가 아닌 날의 복습도 행으로 남으므로
  (`scheduled: false`) 전체 이력을 세면 전체·별표 덱 반복이 창 판정에 섞인다. `counted` 컬럼이
  먼저 필요하다.
- `A-115`의 대응에 뿌리를 적었다 — 화면마다 `readOnly`를 다시 묻는 대신 `useProgress()` 값에
  `canWrite`를 싣는다.
- `A-142`를 열었다: 가린 메모를 `없는 메모`로 말하는 자리가 넷 남았다(대리 보기에서만 거짓).
- `A-146`을 열었다: 학생 홈만 `todayISO()`를 `useMemo` 의존성에 싣지 않아 자정을 넘긴 세션이
  어제 개수를 말한다(`learn.tsx`에서 고친 것과 같은 결함).


## 2026-08-19 — 학부모 리포트·자녀 상세의 열둘을 고친다 (D-178)

검토 결과를 받아 하나씩 코드에서 확인하고 고쳤다. **재현되지 않은 것과 결정 대기에 걸린 것은
고치지 않았다**(아래 `고치지 않은 것`).

### 사실을 다르게 말하던 것

1. **홈이 개인·학원을 합친 정답률을 말했다.** `app/parent/index.tsx`의 자녀 줄이
   `정답률 ${r.totals.accuracy}%`를 붙였는데 그 값은 `statOf(month, allRows)`가 두 출처를 합쳐
   낸 것이다 — D-050·§19는 정답률을 각 출처 블록에만 두라고 정했고 리포트 본문은 실제로 그렇게
   말한다. 홈에서 그 조각을 뺐다(계산은 그대로 — 화면이 부르지 않을 뿐이다). 합쳐도 사실인 것
   (공부한 날·안 낸 과제·학습 기록 없음)만 남는다. `e2e/parent-flow.spec.ts:181`은
   `(\d+월)에 (\d+)일 공부했어요`만 단정해 영향이 없다.
2. **`제재·갈래별 성취도`가 합친 사실을 말하지 않았다.** `report.ts`의 `byTopic`은 출처 구분 없이
   누적하는데, 이 섹션이 학원 전용 블록(`학원 과제 현황`·`반에서`) 바로 아래에 있어 학원 성적으로
   읽혔다. 캡션 앞에 리포트의 `영역별 정답률`이 이미 쓰던 문장을 **그대로** 붙였다 —
   `학원 과제와 개인 학습을 합쳐 셌어요.` 새 문구를 만들지 않았다. 계산은 손대지 않았다.
3. **모르는 `?child=`를 조용히 첫 자녀로 바꿔 열었다.** 주소에 적힌 자녀와 화면이 말하는 자녀가
   다르면 학부모는 알 방법이 없다. `app/parent/detail.tsx`가 같은 자리를 정확히 갈라 말하고
   있어서 그 갈래를 그대로 썼다(`자녀를 찾을 수 없어요` + `연결된 자녀만 볼 수 있어요.` +
   내 자녀 리포트로 가는 길). **쿼리가 아예 없으면 지금처럼 첫 자녀를 연다** — 홈에서 들어오는
   정상 경로다.

### 고른 것이 남지 않던 것

4. **달과 자녀가 주소에 남지 않았다.** 학부모 라우트는 `<Slot />` 하나라 `자세히 보기`로 넘어가는
   순간 리포트가 언마운트된다 — 뒤로 나오면 달 없는 앞 주소로 돌아가 방금 보던 6월이 이번 달로
   되돌아갔고, 새로고침은 늘 첫 자녀를 열었다. 둘 다 `router.setParams`로 옮겼다. **히스토리를
   늘리지 않는다** — 라우트 키가 그대로라 `expo-router`의 `useLinking`이 `history.replace`로
   내려보낸다(`build/fork/useLinking.js`의 `historyDelta === 0` 갈래). 받는 쪽은 이미 준비돼
   있었다(`report.tsx`가 두 쿼리를 읽고 `buildChildReport`가 모르는 달을 이번 달로 버린다).
   자녀를 바꿀 때는 달을 함께 비운다 — 달은 그 자녀의 기록에 붙은 값이다.

### 판단에 쓰는 값이 화면에서 가장 흐렸다

5. **`/parent/attempt`의 값 다섯이 전부 `Row.meta`였다**(제출한 날·마감·걸린 시간·반 순위·문항당
   평균). `meta`는 `inkTertiary`(라이트 대비 2.96:1, AA 미달)이고 §8·§19·§20이 이미 `판단에 쓰는
   값은 `trailing`의 `label``이라고 정해 두었다. `app/parent/children.tsx`의 `Val`과 같은 형태를
   화면 안에 두고 옮겼다. `meta`에는 분류(`국어 · 문학`)만 남았다.
6. **읽어야 하는 고지·안내 열둘이 `tone="tertiary"`였다**(§3 · D-166). 리포트 일곱 자리와 상세
   다섯 자리를 `secondary`로 옮겼다. 장식·목업성 문장과 목록 안 부가 정보는 그대로 뒀다 —
   토큰 자체를 어떻게 할지는 A-123에 열려 있다.

### 누를 수 있다는 사실 · 터치 하한

7. **학습 기록 행이 누를 수 있다는 신호가 없었다.** `trailing`이 재풀이 토글이라 chevron을 둘 수
   없고(§8), 누를 수 있는 행과 없는 행이 완전히 같은 모양이었다 — `/parent/attempt`로 가는 길은
   레포에서 이 행 하나뿐이다. ①섹션 캡션에 `줄을 누르면 문항별로 볼 수 있어요.`를 두고(누를 수
   있는 행이 하나라도 있을 때만) ②`hasDetail`이 거짓인 행은 `subtitle` 끝에
   `문항 내역이 남아 있지 않아요`를 붙였다 — 그 화면이 같은 사실을 말할 때 쓰는 문장과 같다.
8. **칭찬 고르기 다섯 버튼이 32px이었다.** 한 번 누르면 되돌릴 수 없는 쓰기가 자녀에게 간다.
   기본 크기(44)로 올리고 `닫기`를 **선택지 뒤**로 보냈다(§8 ③ — 취소가 고를 것보다 앞에 오면
   먼저 읽힌다). 빈 달 화면의 `{N}월 리포트 보기`도 44로 올렸다(그 화면의 행동이다).
   `sm`은 §8 R2대로 섹션 제목 옆에만 남는다(`자세히 보기`·`N개 더 보기`·`다시 요약하기`).
9. **막힌 달 화살표가 이름을 잃고 `opacity: 0.4`로 흐려졌다.** 막힌 상태에도 이름을 준다
   (`더 이전 기록이 없어요` · `더 최근 기록이 없어요`), 불투명도는 걷어내고 상태는 색 토큰만으로
   말한다(§3 · §11). `disabled`는 그대로라 E2E의 `toBeDisabled()` 단정이 산다.
   `src/components/Pager.tsx`는 범위 밖이라 손대지 않았다.

### 없는 길 · 빈 카드 · 갈린 형식

10. **`자세히 보기`가 그 달에 푼 학습이 있을 때만 있었다.** 상세의 `학원 과제 현황`은 배정만 보고
    그리므로, 과제를 받아 두고 아직 풀지 않은 자녀의 리포트에는 상세로 가는 길이 아예 없었다.
    조건을 `푼 학습 > 0 ∨ 배정 > 0 ∨ 담은 오답 > 0`으로 넓히고, 빈 달에서는 `{N}월 리포트 보기`와
    한 줄에 나란히 둔다(둘 다 44).
11. **상세가 `이 달 학습 시간`·`이 달에 담은 오답`을 고정 문자열로 말했다.** 이 화면은 `month`
    쿼리로 고정된 달을 보여 주고 다른 문장은 모두 `{r.label}`을 쓴다 — 그 달 이름으로 맞췄다.
    담은 오답이 하나도 없는 달에는 `0개 · — · 0개` 세 줄 대신 리포트와 같은 문장 한 줄을 둔다(§9).
12. **`fmtTime`이 세 벌이었고 그중 하나만 시간 분기가 없었다.** 같은 4,512초가 상세에서
    `1시간 15분`, 그 학습 상세에서 `75분 12초`였다. `src/features/learning.ts`에
    `formatDuration` 하나를 두고(형식은 시간 분기가 있는 쪽이 정본) 학부모 화면 셋이 부른다.
    학생 화면 둘(`records.tsx`·`result/[id].tsx`)은 이번 범위가 아니라 A-147로 열어 두었다.

그 밖에 `LoadFailed`에 `again`을 넘겼다(리포트·상세 — 둘 다 **값을 이미 그리고 있는 갈래**다.
손에 아무것도 없는 갈래에는 붙이지 않는다), 학부모 홈의 `읽는 중` 한 줄을 §9의 형태로 맞췄고,
같은 값을 다르게 부르던 두 자리를 통일했다(`오늘까지 담긴 오답` → `지금까지` ·
`5명 미만인 과제는` → `5명보다 적은 과제는`).

### 고치지 않은 것

- **`band()`(상위권·중위권·하위권)** — M-DEC-7(결정 대기). 지시대로 손대지 않았다.
- **`onTime`·`reportDueOf`의 마감 기준** — M-DEC-8(결정 대기).
- **`detail.tsx`의 `재풀이 요청 결과 안내`** — 그런 문장이 이 화면에 없다. 지적된 줄 번호(약 167)에
  있는 것은 요일 분포 캡션(`마감이 어느 요일에 몰렸는지 봐요.`)이라 그것을 `secondary`로 옮겼다.
- **`praise-open`(칭찬 보내기)의 `sm`** — 지시 범위가 펼친 뒤의 다섯 버튼과 `닫기`였다. 펼치기 전과
  후의 크기가 갈리므로 함께 볼 후보로 남긴다(M-DEC-3의 `sm` 대 44 논의와 같은 자리다).
- `ChildReport`에 남은 `tertiary` 여섯(빈 달 안내 · 담긴 오답 총계 두 곳 · 오답 카드의
  영역·날짜 줄 · `아직 메모를 정리하지 않았어요.` · 달 막대의 일수) — 지시된 목록 밖이고,
  값에 딸린 부가 정보라 판단이 갈린다. A-123에서 함께 본다.
...
| 타입 검사 | `npm run typecheck` | 통과 |
| 린트 | `npm run lint` | 오류 0 · 경고 5(기존: `router.d.ts` 1 · `fonts.web.ts` 4) |
| 단위·통합 | `npm test` | **244 통과 / 244**(14 스위트). 241 → 244(`__tests__/learning.test.ts` 3건) |

`npx playwright test`·`npm run db:seed`·`npm run db:verify`는 **지시대로 돌리지 않았다**(DB 재시드가
필요하다). 그래서 `e2e/parent-flow.spec.ts`에 더한 두 건(주소에 남는 달·자녀, 모르는 `?child=`)은
**정적 근거만 있다.** 기존 단정 중 이번 변경과 닿는 것을 훑었고 바꾼 것은 없다 —
`month-label`·토스트 문구·`report-latest-month`·`praise-*`·`attempt-class`·`detail-*` 모두 그대로다.
화면 확인(3뷰포트·다크)도 하지 않았다 — 요청자가 이어서 한다.

### 화면으로만 나온 둘 (같은 작업, 코드 검토가 못 본 것)

**① `영역별 정답률`에 이름 없는 막대가 서 있었다.** 정예린 8월 리포트를 1280에서 띄우자
`80% · 10문항` 막대의 라벨 자리가 비어 있었다. 원인은 화면이 아니라 **권한**이다 —
`content_sets_select` 정책에 학부모 절이 없다(공개 세트 · 운영자 · 자기 학원 · 배정 대상 학생 ·
담당 반뿐). 그래서 학원이 학생에게 공개하지 않은 세트(`publish_to_students = false`)는 학부모의
조회에서 조인이 비고 `area`가 `''`가 된다(`src/repo/learning.ts`의 `?? ''`). 실측으로 확인했다:
정예린의 학원 제출 4건 중 `현대소설 - 인물의 심리` 하나만 비공개라 그 하나만 영역을 잃었다.

빈 영역을 빼고 세되 **뺀 사실을 화면이 말한다** — `학원이 공개하지 않은 학습 1개는 영역을 알 수
없어 빼고 셌어요.`(`area-unknown`). 마감일 없는 배정을 다루는 방식과 같다. 정책 자체를 고치는 것은
학원 콘텐츠 본문을 학부모에게 어디까지 여는가의 문제라 여기서 하지 않았다 — **M-DEC-9**로 열었다.

**② 칭찬을 펼치면 화면에 설명이 하나도 없었다.** 접힌 상태에서는 버튼 라벨(`칭찬 보내기`)이 곧
제목인데, 펼치면 그 버튼이 사라지고 선택지 넷 + `닫기`만 남는다(실측 390: 무엇에 대한 고르기인지
말하는 글자 0개). 게다가 이 고르기는 한 번 누르면 자녀에게 가고 되돌릴 수 없다(A-045).
`고르면 자녀 홈에 바로 떠요. 보낸 뒤에는 지울 수 없어요.` 한 줄을 선택지 위에 뒀다.

### 화면 확인

| 뷰포트 · 테마 | 본 것 |
|---|---|
| 1280 라이트 | 홈(합친 정답률이 사라졌다) · 리포트 · 상세(7월) · 달 왕복 |
| 820 라이트 | 자녀 상세 — `제재·갈래별` 캡션이 합친 사실을 먼저 말한다(`docs/evidence/parent-detail-820.png`) |
| 390 라이트 | 칭찬 펼침 — 44px 두 줄 + `닫기`가 뒤(`parent-praise-390.png`) · 학습 상세의 값이 오른쪽 label |
| 390 다크 | 리포트 전체 — 막대·칩·강조색 모두 읽힌다(`parent-report-390-dark.png`) |

**달 왕복을 실제로 확인했다**: `‹`를 누르면 주소가 `?month=2026-07`이 되고, `자세히 보기` →
`뒤로`에서 `/parent/report?month=2026-07` · 라벨 `7월`로 돌아온다. 예전에는 이번 달로 리셋됐다.

**남긴 것**: 390에서 학습 기록 행의 부제(정답률·영역·문항·날짜·순위 다섯 조각)가 3줄로 감기고
`8월 17`과 `일` 사이에서 끊긴다 — A-148.

### 최종 검증 (D-177 정리 · D-178 함께)

| 목적 | 명령 | 결과 |
|---|---|---|
| 타입 검사 | `npm run typecheck` | 통과 |
| 린트 | `npm run lint` | 오류 0 · 경고 5(기존: `router.d.ts` 1 · `fonts.web.ts` 4) |
| 단위·통합 | `npm test` | **244 통과 / 244**(14 스위트) |
| 스케줄 RPC·권한 | `npm run db:verify:schedule` | **통과 85 / 실패 0**(84 → 85, `GRADUATE_STREAK` 단정 추가) |
| 사용자 흐름 | `npx playwright test`(3뷰포트) | **589 통과 / 20 실패** |

**20건은 기준 실패 6종(×3뷰포트) + 흔들림 2건이다. 새 회귀는 없다.**

기준 실패 6종을 이번에 근거까지 확인했다 — 예전 기록은 줄 번호만 남아 있었다.

| 실패 | 화면이 실제로 보여 준 것 | 뿌리 |
|---|---|---|
| `boundary-flow:86` | 연결을 끊어도 `현대소설 점검`이 1개 남는다 | M-DB-8 |
| `parent-flow:312` · `queue-flow:225` | `이미 제출한 학습이에요` + `다시 풀기`라서 `detail-start`가 없다 | seed가 정예린의 배정 4건을 모두 제출로 만든다 |
| `student-flow:266` · `:286` | `detail-retry`를 기다리다 30초 | 같은 뿌리 |
| `student-flow:825` | `/signup`에서 멈춘다 | 가입이 막힌 M-DB-2 |

흔들림 2건은 **DB 재시드 경합**이다 — `[mobile] recommend-flow:16`은 `_seed.ts:56`에서
`error: deadlock detected`로 죽었고, `[mobile] queue-flow:50`은 그 뒤 담아 두기 체크박스를 기다리다
타임아웃했다. 둘을 단독으로 다시 돌려 **2/2 통과**를 확인했다. 앱 코드가 아니라 워커 사이의
재시드 충돌이고, `e2e/_fixtures.ts`가 경고해 둔 그 자리다.

기준선이 8건에서 6건으로 줄었다 — AI 상한에 걸리던 `parent-flow:541`과 오답노트 지문 펼치기가
이번 실행에서 통과했다.

## 2026-08-19 — 학습 기록·성취 시스템 (D-179)

사용자 지시: 학생의 실제 학습 활동이 장기간 기록으로 쌓이고, 학생에게는 성취감을 학부모에게는
자녀가 얼마나 꾸준히 공부했는지를 보여 주는 본격적인 학습 기록·성취 시스템. 요구사항 원문은
`docs/scody-learning-record-system.md`(축약 없이 저장한 구현 명세)이고, 채택한 설계와 구현 결과는
그 문서 뒤쪽에 함께 남겼다.

### 먼저 코드에서 확인한 것

- **누적을 셀 수 있는 자리가 없었다.** `useProgress().attempts`는 **최신 회차만** 남기고
  (`loadAttempts`가 `attempt_no`로 접는다) 오답 복습·학습 시간은 그 값에 오지 않는다. 기록 화면의
  `총 학습 시간`은 그 접힌 목록의 `timeSec` 합이었다.
- **`attempts.time_sec`이 벽시계였다.** `app/student/solve/[id].tsx`가 마운트 시각(`startRef`)을
  한 번 잡고 제출에서 뺐다 — 탭을 세 시간 열어 두면 `10800`이 기록된다. 그리고 **제출한 학습에만
  붙는다**: `note_reviews`에는 시간 컬럼이 없고, 절반 풀고 다음 날 낸 학습은 시간 전부가 낸 날로
  몰린다. 학부모 화면이 이 값을 `실제 학습 시간`이라 부르면 거짓이 된다.
- **`learning_events`는 있지만 지표용이다.** `answer_saved`(하루 한 줄)는 운영자의 `활성`
  정의이고(D-1) `공부했나`와 다른 질문이다. 앱 실행은 이벤트로 남지 않는다는 규칙은 그대로 쓸 수
  있었다.
- **`state = 'graduated'`로 익힘을 세면 줄어든다.** 익힘은 30일마다 유지 복습을 받고 그 복습에서
  틀리면 `rpc_log_note_review`가 `queued`로 되돌린다(0040) — 어제 50개였던 학생이 오늘 49개가 된다.
- `app/student/records.tsx`는 이미 있었지만 `완료한 학습` 목록 + 정답률 카드였다.

### 만든 것

| 층 | 무엇 |
|---|---|
| 원천(0043) | `study_activity`(활동이 있었던 학습 시간 조각 · append-only · `rpc_log_study_time`이 유일한 문 · 한 번 900초 · 하루 8시간으로 **깎는다**) · `wrong_notes.graduated_on`(익힘에 처음 닿은 날 · 가드 트리거가 세우고 클라이언트 쓰기를 막는다 · 지난 데이터는 복습 로그의 마지막 정답일로 백필) · `attempts.time_sec` 상한 트리거 |
| 일별 집계(0044) | `v_daily_learning_stats` — 하루당 문항·정답·세트·오답 복습·맞힘·담은 오답·익힘·학습 시간·채점 문항·학습일 여부. **같은 날 같은 학습을 여러 번 내면 최신 회차 하나만 센다** |
| 묶음(0044·0045·0046) | `rpc_student_records`(오늘·연속·누적·최고·오늘 뺀 최고·이번 주·지난주·최근 4주 평균·잔디 28일·최근 8주) · `rpc_readable_records`(본인 + 연결된 자녀 — **대상도 서버가 정한다**) |
| 측정 | `src/features/activeTime.ts` — 마지막 활동 뒤 60초까지만, 화면이 앞에 있을 때만, 한 tick 최대 2초(백그라운드 스로틀 방어). 60초마다 증분만 보내고 화면을 떠날 때 남은 것을 보낸다 |
| 판단 | `src/features/records.ts` — milestone 네 축(연속·문항·익힘·학습일) · 오늘 넘은 것 · 새 기록 판정 · 지난주/최근 4주 비교 문장 · 꾸준함 |
| 화면 | 홈(인사 아래 조용한 한 줄) · 결과(오늘의 기록 + 축하 최대 둘) · 기록 탭(나의 기록 7섹션 + 기존 목록) · 학부모 리포트(`StudyProof` — 이번 주 학습 증명) |
| 컴포넌트 | `CountUp`(모션 줄이기에서 즉시) · `DayHeatmap`(3단계 · 요일에 맞추지 않는다) · `RecordHighlight` · `StudyProof` |

### 구현하면서 고친 것

- **0044의 기록 보호가 일요일에 결산했다.** `월~금 5일을 채우고 토요일 결석`에서 보호가 생기기
  **전에** 연속이 끊긴다 — 실측으로 `current`가 3으로 떨어졌다. 0046이 목표를 채우는 순간 주도록
  고쳤고, 검증 배열이 그 차이를 잡는다(보호 없으면 3, 있으면 9).
- **`verify-records.ts`의 첫 정리가 seed 행까지 지웠다.** `delete … where student_id = yerin`이
  seed가 넣은 학습 시간을 함께 지워, 두 번째 실행이 seed 상태가 아닌 DB에서 돌았다. `max(id)`
  기준선 뒤에 생긴 행만 지우도록 고쳤다(연속 두 번 실행에 같은 값을 확인했다).
- **`formatDuration` 사본을 만들었다가 지웠고, 그 함수를 `clock.ts`로 옮겼다.** 같은 이름이
  `src/features/learning.ts`에 이미 있었다(D-178이 학부모 화면 세 벌을 그 하나로 모은 함수다).
  사본을 지우고 학생 화면 둘의 `fmtTime`도 그 함수로 옮겨 **A-147을 함께 닫았다**.
  그런데 `records.ts`가 `learning.ts`를 가져오자 **`scripts/verify-records.ts`가 죽었다** —
  그 파일은 훅을 담아 react-native을 끌어오고 `tsx`가 `react-native/index.js`를 변환하지 못한다
  (실측: `Transform failed … Unexpected "typeof"`). 검증 스크립트가 `STUDY_DAY_QUESTIONS`를
  가져오는 경로라 이것이 실제 결함이다. 순수 시간 유틸인 `src/features/clock.ts`로 구현을 옮기고
  `learning.ts`는 다시 내보낸다 — 화면들의 import 경로는 그대로다.
- **seed가 `실제 학습 시간` 0분을 만들었다.** 새 표가 truncate 목록에 없고 값도 없어서 기록·증명
  화면이 통째로 빈 것처럼 보였다. 풀이의 걸린 시간과 오답 복습 시간(카드당 90초)을 그 날의
  학습 시간으로 넣고, `graduated_on`도 복습 로그의 마지막 정답일로 직접 쓴다(트리거에 맡기면
  몇 주 전에 익힌 오답이 `오늘 익힘`이 된다).

### 검증

| 목적 | 명령 | 결과 |
|---|---|---|
| 기록·집계·연속·상한·권한 | `npx tsx scripts/verify-records.ts` | **52 통과 / 0 실패** (연속 두 번 실행 동일) |
| RLS | `npm run db:verify` | **176 통과 / 0 실패** |
| 복습 스케줄 | `npx tsx scripts/verify-note-schedule.ts` | **85 통과 / 0 실패** |
| 단위·통합 | `npm test` | **282 / 282** (새 38건) |
| 타입·린트 | `npm run typecheck` · `npm run lint` | 통과 (경고 5건은 기준) |
| 웹 빌드 | `npm run web:export` | 성공 (2.8MB) |
| 사용자 흐름 | `npx playwright test`(3뷰포트) | **588 통과 / 21 실패** |

**21건 중 18건이 기준 실패다** — 문서에 남아 있던 6종(`boundary-flow:86` · `parent-flow:312` ·
`queue-flow:225` · `student-flow:266` · `:286` · `:825`) × 3뷰포트이고 뿌리는 M-DB-8 · seed가
정예린의 배정을 모두 제출로 만드는 것 · 가입이 막힌 M-DB-2다.

**나머지 3건은 흔들림이다.** `[mobile] student-flow:235` · `[mobile] student-flow:494` ·
`[tablet] admin-flow:649`를 각각 단독으로 다시 돌려 **3/3 통과**를 확인했다(`:494`는
`learn-grade-` 클릭에서 타임아웃했다 — `e2e/_fixtures.ts`가 경고해 둔 재시드 경합 자리다).
**새 회귀는 없다.**

### 기준 결함을 하나 분리해 기록했다(A-151)

`scripts/verify-note-schedule.ts`의 정리가 `delete from note_reviews where note_id = $1`로
**seed가 넣은 복습 로그까지** 지운다. 그래서 그 스크립트를 돌린 뒤 `verify-rls.ts`가 3건
실패한다. 인과를 분리해 확인했다 — 재시드 직후 `verify-rls` 176/0 → `verify-note-schedule` 85/0
→ `verify-rls` 3건 실패. **D-179의 변경과 무관하고** 이번 범위가 아니라 손대지 않았다.
`npm run db:verify`가 앞에서 재시드하므로 문서화된 순서로는 드러나지 않는다.

## 2026-08-19 — 학생 기록 흐름의 열둘을 고친다 (D-180)

D-179로 만든 기록·성취 시스템을 학생 흐름으로 다시 훑어 나온 열둘을 닫았다. **지적을 그대로
믿지 않고 코드와 브라우저에서 먼저 확인했다** — 잔디 칸 크기·중간 단계 대비·홈 기록 줄 간격은
숫자로 재현한 뒤 고쳤다.

### 무엇이 문제였나 (한 규칙이 여섯을 설명한다)

**같은 이름 · 같은 색 · 같은 수를 두 번 쓰지 않는다.**

- `완료한 학습`이 한 화면에서 **세 가지 수**의 이름이었다. `지금까지` 섹션의 누적 행은 서버 뷰의
  `(학생, 제출일, 출처, 배정/세트)`별 1건이라 **다른 날 다시 푼 세트를 다시 세고**
  (`0044_learning_records.sql`가 그것을 의도라고 밝힌다), `평균 정답률`의 부제가 말하는 수는
  provider가 `itemId`당 최신 회차만 남긴 **세트 수**이고, 아래 섹션 제목도 같은 이름이었다.
  → 누적 행을 **`제출한 학습`**으로 갈라 정의를 부제에 적고, 평균 정답률의 부제가 세는 대상을
  이름으로 못박는다(`아래 완료한 학습 N개`). 아래 섹션 제목은 그대로 둔다 — E2E와 4절이 쓴다.
- `연속 학습` 줄이 `subtitle`(`17일째 공부 중`)과 `trailing`(`17일`)에서 같은 수를 두 번 말했다.
  그리고 `current === 0`이면 문장에는 수가 없는데 `trailing`은 `0일`이었다.
  → `streakLine`에서 수를 빼 조건만 말하게 하고(`오늘도 공부했어요` / `오늘 3문항을 풀면 이어져요`)
  값은 `trailing`이, 0이면 아무것도 말하지 않는다. **값의 자리가 없는 홈**은 그 화면이 앞에
  `N일 연속 ·`을 붙인다(`streakSummary`) — 화면에 수는 여전히 한 번이다.
- 축하 블록(`RecordHighlight`)의 `accentSoft` 면 + `accent` 테두리가 **이 앱의 `선택됨/켜짐`과 같은
  한 벌**이었다(§16의 고른 선지 · `IconButton`의 켜진 상태 · 활성 탭). 결과 화면 아래쪽에는 그 켜진
  `IconButton`(오답노트 담기)이 실제로 서 있다 — 방금 그 옷을 `내가 고른 답`으로 배운 학생에게
  화면에서 가장 눈에 띄는 블록이 눌리는 것처럼 보이는데 눌러도 아무 일이 없었다(§8이 금지한다).
  → `surface` + `border` + `radius.lg`(= `ScoreCard`와 같은 형태)로 바꾸고 강조색은 `eyebrow`에만
  남겼다. 그 면 위 `inkSecondary`는 **라이트 7.34:1 · 다크 7.91:1**이다(고치기 전 `accentSoft`
  위에서는 6.25 / 6.58).
- `다가오는 기록`만 `tone="accent"`였다. §18-2가 그 표현을 `고른 항목`에 배정해 두어 **누를 수 있는
  줄**로 읽혔다(그 줄은 눌리지 않는다). → 기본 톤 + `weight="semibold"`.
- 머리글자가 `지금까지` 섹션 **9행 중 4행**을 같은 포맷터로 되풀이했고, `지금까지`라는 같은 라벨이
  eyebrow와 섹션 제목(h2) 두 곳에 있었다(스크린리더로 제목을 훑으면 나오는 `지금까지` h2가 큰
  숫자가 있는 블록이 아니다). → eyebrow는 `쌓아 온 기록`, 캡션은 아래 어느 줄도 말하지 않는 사실
  (`7월 17일에 시작했어요`)로 바꿨다. 큰 숫자는 `AppText variant="display"`가 정하고, 손으로 쌓아
  **D-166이 걷어낸 자간(-0.6)을 이 화면에만 되살린** `headlineNumber`를 지웠다.

### 장식이 내용보다 커진 것

- **잔디 칸에 상한이 없어 컬럼 폭이 곧 칸 크기였다.** 실측: 390에서 47.7px · 820·1280에서 **93.7px**,
  블록 높이 387px — 이 화면에서 가장 큰 요소가 잔디였다. 상한을 **48px**로 두고(모바일에서 컬럼을
  꽉 채우는 크기다) 줄을 왼쪽으로 붙였다. 고친 뒤 48 / 48 / 47.7 · 블록 232px이고, 같은 컴포넌트를
  쓰는 학부모 리포트도 48px이다(실측).
- **중간 단계가 색으로만 갈려 있었다.** `offset`(빈 칸)과 `accentSoft`(옅은 칸)의 명도 대비가
  **라이트 1.01:1 · 다크 1.07:1**이다 — 색조만 다르고 명도가 같아 셋째 단계가 사실상 없었다(§11).
  면은 그대로 두고 `accent` 1px 테두리로 바꿨다(`offset` 위 3.85:1 / 5.27:1 — 비문자 3:1을 넘는다).
  캡션이 두 단계를 다 말하고 `accessibilityLabel`이 중간 단계 일수를 함께 센다. **0인 단계는 세지
  않는다** — `todayLine`과 같은 규칙이다.

### 규칙을 아는 사람이 화면에 없던 것

- **측정 방식 고지가 학부모 화면에만 있었다.** 행동을 바꿔야 하는 쪽은 학생인데 규칙은 학생만
  몰랐고, 그러면 `학습 시간 3시간`은 근거 없는 수치다. 문구를 `src/features/records.ts`의 상수 둘
  (`STUDY_METHOD_NOTICE_STUDENT` · `STUDY_METHOD_NOTICE_PARENT`)로 옮기고 두 화면이 그것을 쓴다
  (`ACADEMY_MEMO_NOTICE`의 선례). 학습일 기준은 `STUDY_DAY_QUESTIONS`에서 와서 DB 값과 갈리지 않는다.
- **기록이 없는 계정에서 기록 탭이 기록 시스템을 통째로 감췄다.** 게이트가 `firstDay`라 여덟 섹션이
  사라졌는데, 홈은 그 계정에도 기록 줄을 그리고 `나의 기록 보기`로 이 화면에 보낸다 — 도착한 화면에
  기록이라는 개념이 한 글자도 없었다. 빈 카드 여덟 개를 남기지 않는 판단(§9)은 그대로 두고 그 자리에
  **고지 한 줄 + `다가오는 기록` 세 줄**을 둔다(그 계정에서도 `upcomingMilestones`는 값을 만든다).
  **행동은 더하지 않았다** — 화면의 행동은 `완료한 학습` 빈 상태 하나다(`records-empty-start`는
  개인 이용권이 없으면 렌더되지 않는다, D-141).
- **홈 기록 줄이 `tap.textLine`의 음수 마진으로 덩어리 간격을 먹었다.** 그 상수의 `-12`는
  `(44 − 20) / 2` 즉 **캡션 한 줄** 기준인데(`src/theme/styles.ts`가 그렇게 적어 두었다) 이 줄은
  캡션 둘(약 42px)이다. `minHeight`만 주는 쪽으로 바꿨다 — 실측: 두 줄 상태에서 높이 44px · 위
  간격 모바일 16 · 데스크톱 24(고치기 전에는 4 · 12였다). 한 줄인 날도 44px로 같다.
- **눌리는 줄의 신호가 이 화면에서 가장 흐린 색이었다.** chevron이 `size 14` + `inkTertiary`
  (§3이 `bg` 위 2.96:1로 실측해 둔 토큰)였고 정작 눌리지 않는 아래 줄들은 전부 강조색이었다.
  `Row`와 같은 `size 18` + `inkSecondary`로 올렸다. **강조색으로는 바꾸지 않았다** — 넷 다
  알림처럼 보인다(D-179의 판단).
- `기록 보호` 부제만 두 문장이고 이 화면 `Row.subtitle` 중 유일하게 마침표가 있었다. 한 문장으로
  줄였다(`한 주에 5일을 채우면 생기고 하루 빠진 날을 메워요`).

### 고치지 않은 것

- **`이번 주` 섹션의 비교 기준 둘**(진행 중인 주 vs 완성된 주 · 주 5일 학생의 연속)은 사용자 결정
  대기로 남긴다 — 6절 `결정 대기`에 **M-DEC-10 · M-DEC-11**로 적었다. 하나는 마이그레이션이 필요하다.
- `StudyProof`의 `compare`에 **닿을 수 없는 갈래**가 남아 있다(`if (pct === '지난주와 같아요')`).
  `percentLabel`이 문장을 돌려주지 않게 된 뒤 죽은 코드다 — 이번 범위가 아니라 보고만 한다.

### 테스트에서 고친 단정

- `streakLine` 3건: 문장에서 수를 뺐으므로 새 문장으로 고치고 `not.toContain('17')`을 더했다.
- `percentLabel` 1건: `'지난주와 같아요'` → `'0%'`. **이번 변경과 무관한 기준 실패였다** — 이미
  옮겨진 `percentLabel`이 문장을 만들지 않는데 단정만 옛 동작에 남아 있었다(그 함수의 docblock이
  이유를 적어 두었다). 문구를 만드는 자리는 `weekCompare`다.
...
| 단위·통합 | `npm test` | **290 / 290** (새 8건: `DayHeatmap` 4 · 고지 상수 2 · `milestoneUnit` 1 · 기록 없는 계정 1) |
| 웹 빌드 | `npx expo export -p web` | 성공 |

**화면 확인**(1280 · 820 · 390 + 다크, 근거 이미지는 `docs/evidence/`):

| 무엇 | 이미지 | 실측 |
|---|---|---|
| 기록 화면 위쪽 | `records-{1280,820,390}-{light,dark}.png` | 잔디 칸 48 / 48 / 47.7px · 블록 232 / 232 / 251px |
| 기록 화면 아래쪽 | `records-lower-{1280,820,390}-light.png` · `records-lower-390-dark.png` | `제출한 학습` 부제 · 고지 한 줄 · `다가오는 기록` 단위 |
| 기록 없는 계정 | `records-empty-{1280,820,390}-light.png` · `records-empty-390-dark.png` | 고지 + 세 줄 · `records-empty-start` 0개 |
| 홈 기록 줄 | `home-record-{1280,820,390}-light.png` · `-390-dark.png` · `-{390,1280}-twolines.png` | 두 줄 44px · 간격 16(모바일) / 24 |
| 연속 학습 줄(오늘이 학습일일 때) | `records-streak-390-studyday.png` | `오늘도 공부했어요` + `17일` — 수는 한 번이다 |
| 결과 화면 축하 블록 | `result-record-{390,1280}-light.png` · `result-record-390-dark.png` | `ScoreCard`와 같은 면 |
| 학부모 고지 | `parent-proof-1280-light.png` | 같은 상수 · 잔디 칸 48px |

**두 줄 기록 줄과 축하 블록은 seed에 `오늘` 활동이 없어 응답을 읽는 자리에서 끼워 화면 상태를
만들었다**(`rpc_readable_records` · `attempts` 조회의 **읽기 응답만** 고쳤고 DB는 건드리지 않았다).
그 상태에서 확인한 것은 **간격과 면**이고, 값은 실제 데이터가 아니다.

E2E는 사용자가 3뷰포트로 돌린다.

## 2026-08-20 — 결정 대기 둘을 닫는다: 보호 2개 · 같은 시점 비교 (D-181)

`/product-review 학생 기록 흐름`이 남긴 사용자 결정 두 건을 구현했다. D-180이 나머지 열둘을 닫고
이 둘만 남겨 두었다 — 하나는 확정 정책(D-179 ④)을 건드리고 하나는 마이그레이션이 필요해서다.

### ① 주 5일을 지키는 학생의 연속이 매주 끊겼다 (M-DEC-11 → ②)
...
그래서 앱이 스스로 `주간 목표 5일`이라고 부르는 것을 **매주 100% 지키는 학생**의 최장 연속이
5일에 갇혔고, streak의 첫 기준선인 `7일 연속 학습`이 영구히 도달 불가였다. retention 장치가 가장
성실한 사용 패턴에서 역방향으로 작동한 것이다.

목표를 채운 주가 **주말 이틀만큼**(`c_week_grant = 2`) 준다. 보유 상한은 그대로 2라 **한 주를
통째로 쉬는 것은 여전히 불가능하다** — 토·일에 둘을 다 쓴다. 보호받은 날이 연속을 늘리지 않는
성질도 그대로다(`17일째 공부 중`은 실제 학습 17일이다).

화면 문구도 고쳤다: `하루 빠진 날을 메워요` → `빠진 날을 메워요`. 보호가 둘이 된 뒤 `하루`는
거짓이다.

### ② 진행 중인 주를 완성된 주와 비교했다 (M-DEC-10 → ①)

`week`는 이번 주 월요일부터 **오늘까지**이고 `lastWeek`는 완성된 7일이었다. 그 둘을 나누면
월요일 아침에 `푼 문항 0문항 / 지난주 42문항 / -100%`가 네 줄에 동시에 선다.

**이 레포는 같은 결함을 이미 한 번 고쳤다** — `DESIGN.md` §18-0의 `lastCompleteWeek`(진행 중인
주를 추이에 넣으면 마지막 점이 늘 바닥으로 떨어져 `제출률이 무너졌다`로 읽힌다). 그 선례를
학생·학부모 쪽에 적용했다.

`rpc_student_records`가 **`lastWeekToDate`**를 함께 준다 — 지난주 월요일부터 **같은 요일까지**다.
문구는 `지난주 이맘때`이고 **학부모 학습 증명도 같은 창을 쓴다**(`StudyProof`의 `compare`).

`최근 4주 평균 대비` 줄은 **비율을 없앴다.** 그 값은 완성된 4주의 주당 평균이라 진행 중인 주와
나누면 같은 결함이 돌아오고, 네 주의 같은 시점을 각각 잘라 평균을 내는 것은 그 줄이 답하는
질문(`평소 얼마나 하나`)에 필요하지 않다. 이름을 **`평소 한 주`**로 바꾸고 두 수를 나란히 적는다.

### 함께 고친 것

`StudyProof.compare`에 **닿을 수 없는 갈래**가 남아 있었다 — `if (pct === '지난주와 같아요')`.
`percentLabel`이 문장을 만들지 않게 된 뒤(같은 함수가 지난주 비교와 4주 평균 비교 두 곳에 쓰여
`최근 4주 평균 대비` 줄이 `지난주와 같아요`라고 말하던 결함을 고칠 때) 죽은 코드가 됐다.
D-180이 범위 밖이라 보고만 했고, 이번에 그 함수를 고치면서 함께 지웠다.

### 검증

| 목적 | 명령 | 결과 |
|---|---|---|
| 기록·집계·연속·상한·권한 | `npm run db:verify:records` | **58 통과 / 0 실패** |
| 단위·통합 | `npm test` | **290 / 290** |
| 타입·린트 | `npm run typecheck` · `npm run lint` | 통과 (경고 5건은 기준) |
| 웹 빌드 | `npm run web:export` | 성공 |
| 사용자 흐름 | `npx playwright test`(3뷰포트) | **587 통과 / 22 실패** |

**22건 중 18건이 기준 실패다**(6종 × 3뷰포트: `boundary-flow:86` · `parent-flow:312` ·
`queue-flow:225` · `student-flow:266` · `:286` · `:825`). 나머지 4건은 흔들림이고 단독 재실행으로
**4/4 통과**를 확인했다 — `[desktop][tablet] queue-flow:50` · `[tablet] student-flow:1434` ·
`[mobile] auth-flow:449`. **실행마다 대상이 바뀐다**: 직전 실행에서는 `[mobile] student-flow:235`·
`:494`와 `[tablet] admin-flow:649`가 같은 방식으로 흔들렸다. 뿌리는 문서에 이미 있는 둘이다 —
워커 사이의 재시드 경합(`e2e/_fixtures.ts`)과 GoTrue 로그인 rate limit(M-DB-15). **새 회귀는 없다.**

화면 확인(1280 · 390 · 라이트·다크): 머리글자 `쌓아 온 기록 / 6 / 일 공부했어요 /
7월 17일에 시작했어요`(중복 없음) · `푼 문항 · 지난주 이맘때 35문항 · -71%`(고치기 전 `지난주
65문항 · -85%`) · `평소 한 주 · 16문항`(비율 없음) · `기록 보호 … 빠진 날을 메워요` ·
잔디 칸 **48px**(1280에서 고치기 전 93.7px · 격자 높이 387→232px) · 세 단계가 테두리로 갈린다 ·
학부모 학습 증명도 `지난주 이맘때`를 쓴다.

검증기에 단정 6개를 더했다. 그중 하나가 **이번 결함의 회귀 검사**다:

> `주 5일을 3주 지키면 최장 연속이 15일이다` — 보호가 하나였다면 5가 나온다.

`current`가 아니라 `longest`로 판정한다. 처음에는 `current === 15`를 기대했는데 실제는 0이었다 —
검증이 이번 주를 심지 않으므로 지난주 토·일을 보호가 메운 뒤 이번 주 월요일부터 끊기는 것이
**정상 동작**이다. 기대가 틀렸고 함수는 맞았다. 사다리가 주말을 건넜는지는 최장 기록이 말한다.

나머지 5개는 `lastWeekToDate`의 창이다 — 시작이 지난주 월요일인지, 끝이 이번 주 경과 일수와
같은지, 그 창의 합이 뷰 행의 합과 같은지, 완성된 지난주가 그 창보다 작지 않은지.

## 2026-08-20 — 개선 후보 다섯을 구현한다 (D-182 · A-149 · A-150 · A-151)

`/product-review`가 남긴 다음 개선 후보 다섯을 사용자 지시로 구현했다.

### ① `verify-note-schedule.ts`가 seed 복습 로그를 지웠다 (A-151 · 기준 결함)

세 자리의 `delete from note_reviews where note_id = $1`이 **seed 행까지** 지우고, `backdate`는 그
노트의 **모든** 행의 `reviewed_on`을 하루씩 뒤로 물렸다. 그래서 그 스크립트를 돌린 뒤
`verify-rls.ts`가 3건 실패했다 — 재시드하면 사라지므로 오래 눈에 띄지 않았다.

**지우기를 좁히는 대신 되돌린다.** 중간 삭제는 각 시나리오의 시작점을 맞추는 의도된 동작이고,
좁히려면 세 곳 모두에 조건을 달아야 하는데 하나만 빠뜨리면 같은 결함이 돌아온다. 시작에서 그
노트의 로그를 통째로 떠 두고(`id`·`reviewed_on`·`recap` 전부) 정리에서 다시 넣으면 `backdate`가
물린 날짜까지 함께 복구된다. 활동 이벤트 트리거는 그 동안 끈다(`learning_events`는 append-only라
앱 역할이 지울 수 없다).

**마지막 단정도 고쳤다**: `남긴 복습 로그가 없다`(0건) → `복습 로그가 seed 상태로 돌아왔다` +
`복습일도 물러나지 않았다`. 예전 단정은 seed 행까지 지운 것을 **통과로 읽었다** — 그것이 이 결함을
스크립트 안에서 보이지 않게 만든 자리다.

인과를 분리해 확인했다: 재시드 → `verify-rls` 176/0 → `verify-note-schedule` **86/0** →
`verify-rls` **176/0**. 고치기 전 마지막 단계가 173/3이었다.

### ② 학습 시간 보관 (A-150 · 절반)

`study_activity`는 append-only이고 클라이언트가 60초마다 조각을 보낸다 — 하루 두 시간이면 약
120행, 상한(8시간)까지면 480행이다. 읽는 것은 **일별 합** 하나뿐인데(`v_daily_learning_stats`)
표는 무한히 자란다.

`compact_study_activity(보관일수)`(0048)가 오래된 날을 `(학생, 날, 종류)`마다 한 행으로 접는다.
**하루의 합은 바뀌지 않고** `ref_id`와 조각 시각만 잃는다(둘 다 되짚기용이고 읽는 화면이 없다).
보관 창의 바닥은 **28일**(잔디가 보는 창)이라 `0`을 넣어도 최근 4주는 접지 않는다. 접기 전후
구간 총합이 다르면 예외로 되돌린다.

한 행의 상한 제약을 `900초`에서 **하루 상한(8시간)**으로 올렸다 — 접은 행이 하루 전체를 담아야
한다. 한 번에 받는 양의 제한은 제약이 아니라 함수가 한다(`rpc_log_study_time`의 `c_flush_cap` ·
직접 쓰기는 회수돼 있어 그 함수가 유일한 문이다).

실행은 `npm run db:compact`다(소유자 접속 — 지우는 함수를 클라이언트에 열지 않는다). 스크립트가
**날짜별** 합까지 대조한다: 함수 안의 검사는 구간 총합이라 날짜가 섞여도 통과할 수 있다.

**남은 것은 스케줄뿐이다.** 이 레포에 스케줄러가 없어(M-DEC-6과 같은 공백) 지금은 손으로 돌린다.

### ③ 보호가 쓰인 사실을 학생에게 말한다

응답에 **남은 개수**만 있었다. 그래서 어제 공부하지 않았는데 `연속 학습 12일`이 그대로인 학생은
그 숫자를 틀린 것으로 읽는다 — 잔디에는 빈 칸이 있고 연속은 줄지 않았는데 둘을 잇는 문장이
없었다. `기록 보호 0개`가 `아직 못 얻었다`인지 `방금 다 썼다`인지도 구분되지 않았다.

`streak.protectedDays`(0049)가 **지금 이어지고 있는 연속을 지킨** 보호의 날들을 준다 — 연속이
끊기는 자리에서 비운다(석 달 전에 쓴 보호를 `방금 하나 썼어요`처럼 말하면 그것도 거짓이다).
화면은 `protectionLine`으로 `8월 16일을 메웠어요 · 한 주에 5일을 채우면 생겨요`를 말한다.
여러 날이면 개수를 함께 말하고, **경고는 두지 않는다**(`다음에 빠지면 끊겨요`는 재촉이다).

잔디에 네 번째 단계를 만들지 않았다 — `DayHeatmap`이 단계를 셋으로 묶은 근거가 그대로 유효하다.

### ④ 주간 추이선 (A-149)

서버가 최근 8주를 이미 내려보내는데 읽는 화면이 없었다. `이번 주` 섹션에 `주당 추이` 한 줄로
그린다 — 선은 `trailing`에 두어 `Sparkline`의 용도(모양만 보여 주고 값은 옆 줄이 말한다)를 지킨다.

**진행 중인 이번 주는 넣지 않는다.** `completedWeekTrend`가 그 규칙이고, 근거는 §18-0이 학원
추이선에 대해 이미 적어 둔 것이다 — 넣으면 마지막 점이 늘 바닥으로 떨어져 `무너졌다`로 읽힌다.
점이 둘 미만이면 선을 그리지 않는다.

### ⑤ `다가오는 기록`을 위로 (D-182)

이 화면에 **돌아올 이유**를 만드는 것은 근접 목표인데(D-179 ③) 그 섹션이 회고 다섯 덩어리 뒤,
모바일 3.5화면 스크롤의 **y≈2,100**에 있었다. §19가 학부모 리포트의 `자세히 보기`를 같은 이유로
y≈2,400에서 y≈374로 올린 선례가 있다.

`오늘` 바로 뒤로 올렸다 — 위 두 섹션이 **지금 상태**와 **다음 한 걸음**이고 그 뒤가 되돌아보는
자리다. 마스터 플랜 4절의 섹션 순서를 함께 고쳤다.

실측(390): `다가오는 기록`이 **y≈2,100 → y=480**. 첫 화면(844px) 안이다. 섹션 y좌표는
`오늘 228 → 다가오는 기록 480 → 최근 4주 733 → 이번 주 1,114 → 지금까지 1,890`.

### 검증

| 목적 | 명령 | 결과 |
|---|---|---|
| 기록·집계·연속·상한·접기·권한 | `npm run db:verify:records` | **72 통과 / 0 실패** (+13) |
| RLS | `npm run db:verify` | **176 통과 / 0 실패** |
| 복습 스케줄 | `npx tsx scripts/verify-note-schedule.ts` | **86 통과 / 0 실패** (+1) |
| 접기 실행 경로 | `npm run db:compact` | 성공(seed에는 접을 것이 없다 — 일별 합 34개 그대로) |
| 단위·통합 | `npm test` | **297 / 297** (+7) |
| 타입·린트 | `npm run typecheck` · `npm run lint` | 통과 (경고 5건은 기준) |
| 웹 빌드 | `npm run web:export` | 성공 |
| 사용자 흐름 | `npx playwright test`(3뷰포트) | **590 통과 / 19 실패** |

**19건 중 18건이 기준 실패다**(6종 × 3뷰포트). 나머지 1건(`[desktop] parent-flow:119`)은 단독
재실행으로 통과했다 — 실행마다 대상이 바뀌는 환경 흔들림이고(재시드 경합 · GoTrue rate limit
M-DB-15) 직전 실행에서는 `queue-flow:50`·`student-flow:1434`·`auth-flow:449`가 흔들렸다.
**새 회귀는 없다.**

화면 확인(390 · 라이트): 보호 문장은 **실제 상황을 만들어** 봤다 — 박도윤에게 `지난주 월~금 +
토요일 비움 + 일요일~어제`를 심으면 잔디에 빈 칸이 있는데 `연속 학습 9일`이고, 그 아래가
`8월 15일을 메웠어요 · 한 주에 5일을 채우면 생겨요 · 1개`로 둘을 잇는다. 부제는 390에서 **한 줄**
(20px)이고 줄바꿈이 없다. 확인 뒤 그 데이터를 지우고 재시드했다.

`주당 추이`는 SVG가 실제로 그려지고 스크린리더 문장이
`주당 푼 문항 최근 4주 추이, 가장 낮은 0문항, 가장 높은 65문항, 마지막 65문항`이다.

## 2026-08-20 · 기록 시스템 정리(`/simplify` 네 관점)

재사용·단순화·효율·고도 네 관점을 병렬로 돌렸다. 중복 제거 후 **26개**가 나왔고 그중 **27개를
적용**했다(같은 자리를 여러 관점이 지목한 것이 7건 — `weekCompare`는 셋, 역할 술어도 셋).
건너뜀 5 · 규모가 커진 뒤로 보류 3 · 에이전트가 근거를 확인하고 `그대로 두라`고 판정한 것 28.

### 실측이 추정을 두 번 뒤집었다

효율 관점은 SQL 항목을 `추정`이라고 밝히며 측정 지점까지 알려 줬다(`verify-records.ts`가 이미
소유자 접속을 열어 두었다). 실제로 재 보니 예상보다 컸다.

**뷰가 학생 필터를 원천까지 내려보내지 못했다.** `0044`는 CTE 다섯 개를 각각 두 번 참조했고
(`all_days`의 union + 아래 `left join`) PG12+는 두 번 이상 참조된 CTE를 인라인하지 않는다 —
그것이 최적화 방벽이라 `where student_id = $1`이 CTE 안으로 못 들어갔다.

```
explain (analyze) select * from v_daily_learning_stats where student_id = <정예린>

고치기 전:  Seq Scan on attempts rows=32 · note_reviews 6 · wrong_notes 11 · study_activity 45
            → 정예린의 10행을 얻는 데 원천 94행 전량 집계 · 12.673 ms
고친 뒤:    attempts rows=7 · wrong_notes 8 · study_activity 15
            → Index Cond: (student_id = …) on wrong_notes_graduated_idx · 0.508 ms
```

효율 관점은 `seed에서는 무료`로 봤는데 **25배**였다. 있는 인덱스 네 개가 하나도 쓰이지 않고
있었다. `union all` + 단일 `group by`로 갈래마다 참조를 한 번으로 만들어 고쳤다(0050).

두 번째는 연속 계산이다. `v_d = any(v_days)`가 하루마다 정렬된 배열을 선형 탐색해서 비용이
기간의 제곱이었다 — 시뮬레이션으로 1년 매일 학습 **66,795회** · 3년 주 5일 **551,232회**.
배열이 오름차순이므로 커서를 함께 전진시켜 `기간 + 학습일`로 만들었다(1년 730회 · 91배).

### 교체 단위가 300줄이었다

`create or replace function`은 본문 전체를 요구한다. 그래서 규칙을 세 번 고치는 동안
(0046 보호 시점 · 0047 보호 2개와 지난주 창 · 0049 보호 사용 날짜) 실제로 바뀐 줄은 49 / 35 / 25인데
매번 약 300줄을 옮겨 적었다 — **약 860줄이 `바뀌지 않았다`는 사실만 다시 적은 SQL**이다.

**`0029`가 그 위험을 이미 기록해 두었다**: 0026에서 본문을 부분만 읽고 옮겨 적어 콘텐츠 소유권
검사와 제출 후 초안·담아둔목록 정리를 하마터면 잃을 뻔했다. 이 시스템은 그 위험에 세 번
노출됐고 매번 눈으로 대조해서 통과했다.

규칙 전용 표는 만들지 않았다(파생값 저장을 피한 0044의 판단을 되돌릴 이유가 없다). 대신
`f_window_sum`(창 하나의 합 8개 — 네 자리에 창만 다르게 적혀 있었다)과 `f_student_streak`
(보호 규칙 상수가 그것을 쓰는 유일한 함수에 모인다)로 쪼갰다. 다음 규칙 변경은 60줄 교체다.
선례는 `0045`가 대상 판정만 하고 계산을 위임한 것이다.

### 가장 값이 컸던 것: 근거가 거짓이 된 주석 셋

| 자리 | 적어 둔 근거 | 실제 |
|---|---|---|
| `StudyProof.compare` | `말투가 다르다 — 그쪽은 학생, 이쪽은 학부모` | 네 갈래 문자열이 **글자까지 같았다** |
| `records.ts` 재수출 | `기록 화면이 두 곳에서 가져오지 않게` | 아무도 그 경로를 쓰지 않았다 |
| `repo/records.ts` 머리 | `읽기는 함수 하나` | 둘이었다(`loadRecords` 미사용) |

셋 다 쓴 시점에는 사실이었고 그 뒤 코드가 바뀌면서 거짓이 됐다. **사본과 낭비는 다음 사람이
발견하지만 근거가 적혀 있으면 아무도 의심하지 않는다** — `formatDuration`이 세 화면에서 갈렸던
경로가 정확히 그것이다.

그 종류의 사본이 둘 더 있었다. `dayLabel`은 `formatDate`의 **다섯 번째** 사본인데 하필
`formatDate`의 docblock이 앞의 0을 떼는 근거로 **이 캡션**을 인용하고 있었다. `shift`는
`addDaysISO`의 **여섯 번째** 사본이고, `clock.ts`가 `여섯 번째 사본을 만들기 전에 둘 중 하나로
모은다`고 적어 둔 자리에 정확히 여섯 번째로 들어갔다.

### 잘못된 깊이 넷

- **`refreshRecords`가 세 번째 경로였다.** provider에 쓰기 헬퍼가 이미 둘 있는데(`mutate`·
  `mutateLocal`) 복습 경로는 전체 재조회가 비싸서 그것을 피했고, 고칠 때 `void refreshRecords()`를
  본문에 손으로 붙였다 — 그러면 같은 갈래의 형제와 앞으로 생길 쓰기가 그 줄을 **기억해야** 한다.
  `mutate`가 refresh를 인자로 받게 하고 `logCard`가 그것을 지난다. 복사된 `denied` 검사도 함께
  사라졌다. 카드 진행을 왕복에 매달지 않도록 기다리지 않는 갱신(`refreshRecordsSoon`)을 이름으로 뒀다.
- **조회 실패 흡수가 오분류를 남겼다.** 기록 조회에만 `.catch`를 붙였는데
  **`loadWeekSummaries`는 모든 역할에서 무조건 도는데 학생 화면에 소비처가 없다** — 그것이
  거절하면 학생 홈이 통째로 빈다. 기록에 적어 둔 근거(`장식이 핵심 흐름의 단일 실패점이 된다`)가
  그 조회에 그대로 적용되는데 갈래는 기록에만 생겼다. `soft()`로 필수/곁가지를 코드에 적었다.
- **학습일 기준이 두 진실을 가졌다.** 같은 종류인 `weekGoal`은 이미 서버가 응답에 싣는데 이 값만
  클라이언트 상수였다. 서버가 `studyDayQuestions`를 싣고 `STUDY_DAY_QUESTIONS`는 검증 스크립트의
  기대값으로만 남는다.
- **`ping()` 손배선이 웹에서 전부 중복이었다** — 훅이 이미 document 리스너를 붙인다. 그중
  `scroll`은 **일 자체가 없었다**: 본문은 `ScrollView` 안이고 `scroll`은 버블하지 않아 document
  리스너에 닿지 않는다. 죽은 등록을 지웠다. props로 넘기는 더 깊은 해결은 훅 API와 두 화면을
  함께 바꿔야 해서 남겼다.
...
| 이유 | 항목 |
|---|---|
| diff 밖으로 번짐 | `Row`에 `value` prop(26벌 · 앱 전체가 쓰는 컴포넌트) · `hasFinalConsonant` 공용화(`subjectParticle`이 변경 밖 · 새 모듈 필요) |
| 의도된 동작을 바꿈 | 카드마다 RPC를 덱 끝으로(2장 만에 나가면 홈이 낡는다 — **그게 원래 Critical이었다**) |
| 얻는 것이 비용보다 작음 | 370줄 3항 연산자 추출(props 6개 표면) · `verify-records` 골격 33회(단정 72개는 그대로 두는 게 맞다) |

보류 3: 뷰를 한 호출에 12번 평가(각 호출이 25배 싸졌지만 횟수는 그대로) · `study_activity`
upsert(하루 480→2행이지만 `append-only` 포기가 대가) · `ping()` props API.

### 검증

| 목적 | 결과 |
|---|---|
| `db:verify:records` | **72 / 0** |
| `db:verify` | **176 / 0** |
| `verify-note-schedule` | **86 / 0** |
| `npm test` | **298 / 298** |
| typecheck · lint | 통과 (경고 5는 기준) |
| `web:export` | 성공 |
| Playwright 3뷰포트 | **584 / 25** |

**25건 중 18건이 기준 실패다**(6종 × 3뷰포트). 나머지 7건은 환경이다 — `queue-flow:50`이 세
뷰포트 전부에서 깨져 제 변경(제출·flush 병렬화)을 의심했는데, **자기 파일 전체를 순서대로 돌리면
`:50`·`:103`이 통과하고 기준 항목 `:225`만 남는다**(30 passed). 이번 실행의 추가 실패에
`auth-flow:252`·`:449`가 섞인 것이 M-DB-15(GoTrue 로그인 rate limit) 서명이고, 오늘 검증기와
브라우저 확인으로 로그인을 많이 해서 한도에 더 가까웠다. **새 회귀는 없다.**

값이 바뀌지 않았다는 근거는 `db:verify:records`의 72단정이다 — 뷰를 재작성하고 함수를 셋으로
쪼갠 뒤에도 항등식(`graded = 풀이 + 복습` · `학습일 ⟺ 3문항`)과 누적·최고·주간이 일별 합과 같다.

## 2026-08-20 · 이메일 인증 전환 · 로그인·가입 재기획 · 데모 계정 6종 (D-184 · D-185)

사용자 지시 넷: 회원가입·로그인 **버튼 디자인**을 요즘 형태로 · 가입·로그인 **과정과 문구
재기획**(둘 다 모범 사이트를 firecrawl로 조사해 흉내낸다) · **휴대폰 로그인 → 이메일 로그인** ·
이메일 로그인 안에 **테스트 계정 6개**(학생2·학부모2·학원2)를 Supabase 실제 데이터와 함께.

### 전제를 먼저 바로잡았다

**휴대폰 로그인은 이미 없었다.** 이틀 전 D-171이 로그인을 `스코디 아이디 + 비밀번호`로 바꾸면서
내렸고, 휴대폰이 남은 곳은 **가입**의 `휴대폰 번호로 가입하기` 하나였다. 사용자에게 확인해
**로그인·가입 둘 다** 이메일로 정했다 — 확정 정책 2절 `로그인 방법`을 개정하는 결정이라
사용자 승인으로 기록한다(D-184).
...
| 사이트 | 형태 |
|---|---|
| Vercel `/login` | `Log in to Vercel` → `Continue with Email` → 로고+`Continue with Google/GitHub` → `Show other options` |
| Linear `/login` | `Log in to Linear` → `Continue with Google/email/SAML SSO` |
| Notion `/login` | `Log in to your Notion account` → `Email`+힌트 → **`Continue`** → `or continue with` |
| Slack `/signin` | `Enter your email to sign in` → 이메일 → `Sign In With Email` → `OR SIGN IN WITH` |
| 오늘의집 | `로그인` / `비밀번호 재설정`·`회원가입` / `SNS계정으로 간편 로그인/회원가입` |
| 인프런 | `Login` / Email·Password·Login / 링크 셋 |

수렴한 여덟 중 이 레포가 **이미 지키던 셋**(라벨 붙은 구분선 · 글자 링크 보조 행동 · 약관 한
문장)은 빼고 다섯을 들여왔다: 제목이 제품명을 담는다 · 이메일 우선 · 수단은 브랜드 마크 +
한 줄씩 · 상단 바 쌍은 낮은 강조 + 채운 알약 · `계속하기`는 쓰지 않는다.

#### 조사와 다르게 간 곳 하나 — 로그인을 1단계로 뒀다

그 사이트들의 `Continue`는 **어느 수단으로 갈지 서버에 물어 갈라 보내는 라우터**다. SSO·Passkey·
OAuth가 여럿일 때 값이 있다. 스코디의 수단은 비밀번호 하나다(`signInWithPassword` 단 하나).
그래서 2단계로 만들면 둘 중 하나가 된다.

① 진짜 판정을 하려면 `이 이메일에 무슨 수단이 있나`를 익명에게 답해야 한다 — A-100이 미해결로
올려 둔 **계정 열거 오라클**을 이메일 키로 하나 더 만드는 일이다.
② 판정 없이 늘 비밀번호 칸을 보이면 **도달만 하는 가짜 단계**이고, 그것이 **D-171이 이틀 전에
없앤 결함**이다(`setStep('code')`가 코드에 없어 인증번호 칸·버튼·`onSubmit`이 죽은 코드인데
`AuthSteps step={1} total={2}`가 다음 단계를 약속하고 `2단계 중 1단계`로 낭독됐다).

조사한 한국어 서비스 둘(오늘의집·인프런)도 한 면이다. 부수로 비밀번호 관리자의 자동 채우기가
동작한다(두 칸이 함께 있어야 한다).

### 가입은 3단계 → 2단계

번호 확인 단계와 인증번호 칸을 없앴다. 메일 발송 provider가 없어서(M-DB-2에 SMS만 적혀 있고
메일은 아예 없다) 유지하면 옛 번호 칸이 하던 일을 되풀이한다 — `hint`가 **"발송은 아직 연결되지
않았어요"라고 고백하는 칸**이었고, 할 수 없는 일에는 칸을 두지 않는다(D-141).

카카오 경로는 이제 이메일·비밀번호를 묻지 않는다. 카카오가 곧 자격 증명인데 `연결은 건너뛴다`고
말한 화면이 **바로 아래에서 비밀번호를 정하라고** 했다.

`AuthSteps` 호출부가 0이 됐지만 부품은 남겼다 — §15가 규칙으로 적어 둔 어휘이고 A-021의 복구
흐름이 단계형이다.

### 이메일 중복 RPC를 만들지 않았다 (A-152)

`profiles`에 **email 컬럼이 없다**(전 migration `grep email` 0건) — 이메일은 `auth.users`에만
있다. 익명이 그것을 묻게 하면 A-100의 오라클을 **더 나쁜 키로** 하나 더 만든다: 번호 열거는
형식 공간을 훑는 일이고 이메일 열거는 **남이 만든 유출 목록을 그대로 넣는 일**이다. 게다가
계정이 어차피 만들어지지 않으므로(M-DB-2) 그 검사의 쓸모가 0이다.

**migration을 한 줄도 만들지 않았다.** D-171과 같은 성질의 변경이다.

### 비밀번호를 두 등급으로 갈랐다

`gen-seed.ts`는 **모든 계정에 같은 평문**을 쓴다. 약한 값을 하나로 두면 `admin`도 그 값이 되고,
그것이 **D-157이 실측으로 닫은 구멍**이다(번들 자격 증명으로 `admin@scody.test`에 로그인해
`rpc_admin_overview`까지 불렸다). M-DB-7 때문에 더 위험하다 — 공개 사이트가 이 개발 프로젝트를
가리켜서 약한 계정은 인터넷에서 도달 가능하다.

| 등급 | 비밀번호 | 환경변수 |
|---|---|---|
| 기존 11종(`admin` 포함) | 16자 이상 | `EXPO_PUBLIC_DEV_LOGIN_PASSWORD` (가드 유지) |
| 데모 6종 | 6자 이상(약한 값 허용) | **`DEMO_LOGIN_PASSWORD`** (접두어 없음) |

**접두어를 뗀 것이 핵심이다.** 데모 진입은 사람이 비밀번호를 **타이핑**하므로 클라이언트가 이
값을 알 필요가 없고, 그러면 번들에 들어갈 경로 자체가 사라진다 — D-135의 상수 접기에 기대는
것보다 강하다. 사용자도 "이메일만 보이고 비밀번호는 안"을 골랐고 두 판단이 같은 곳으로 왔다.

`gen-seed.ts`에 불변식 넷을 세웠다. 주석으로 두면 다음 사람이 `admin`에 `tier: 'demo'`를 붙이는
순간 **조용히** 깨진다.

```
DEMO ≥ 6자 · DEMO ≠ DEV · 데모 등급에 admin 역할 금지
데모 등급은 데모학원 밖 소속 금지 · 데모 학부모의 자녀는 데모 학생만
(덤으로 scody_id·phone 중복을 생성 시점에 잡는다)
```

`DEMO ≠ DEV`가 중요하다 — 같은 값을 양쪽에 붙여넣는 사고가 실제로 있을 수 있는 길이고, 그러면
등급을 가른 의미가 전부 사라진다.

#### 추측 하나를 실측으로 확정했다

사용자가 고른 약한 값이 GoTrue의 최소 길이·유출 비밀번호 검사에 걸리는지가 계획의 미확인 항목이었다.

```
POST /auth/v1/token?grant_type=password  (데모 6종)     → access_token 6/6
POST /auth/v1/token?grant_type=password  (admin, 같은 값) → 거부
```

통과한다. seed가 `crypt(평문, gen_salt('bf'))`로 **해시를 직접** 쓰므로 그 검사들(`signUp`과
비밀번호 변경에서 도는 것)을 지나지 않는다. 그리고 **`admin`은 거부된다** — 두 등급이 실제로 갈렸다.

### 데모 계정 6종

여섯이 서로 맞물리는 **새 학원 하나**(`스코디 데모학원`)를 만들었다.

```
원장 1 · 선생 1(반 담당) · 반 1 · 학생 2 · 학부모 2(각각 학생 하나를 자녀로)
```

선생에게 담당 반을 반드시 준다 — 세션이 선생에게 `teacherId`가 자기인 반만 돌려주므로 담당 반이
없으면 그 계정의 화면이 통째로 빈다.

한빛·새길을 건드리지 않은 것이 이 배치의 목적이다: `verify-rls`의 학원별 단정(한빛 반2·학생14 /
새길 반1·학생2)과 `minji 자녀 풀이 10건`·`yerin 7건`이 전부 살아남는다. **콘텐츠 세트를 새로
만들지 않아** `세트 14`·`문항 192`도 손대지 않았다.

**번호 대역은 `010-6000`이다.** 기존 대역(1000/2000/3000/4000/9000)뿐 아니라 **`010-5000`도 쓸 수
없다** — `auth-flow.spec.ts`가 가입 흐름에서 그 셋을 "아직 가입되지 않은 번호"로 쓴다. 계획에서
그것을 미리 잡았다.

`scodyId`를 짧게 잡은 이유는 사람이 이메일을 **손으로 치기** 때문이다(`student1@scody.test` 19자).

기록 화면이 의미 있게 보이도록 학생1에게 개인 풀이 10건을 넣었다: `-1~-5` 다섯 날이 붙어 연속이
0이 아니고, 28일 잔디에 아홉 날이 들어가고(전부 채점 문항 3개 이상 = 학습일), `-31`이 8주 막대의
먼 쪽을 채워 **주간 비교에 상대**가 생긴다. 시간 합 약 113분. 노트 7건은 다섯 상태를 덮는다
(졸업 · 오늘 볼 것 3 · 멈춘 것 · 정리 안 한 것 · 학원 출처).

**연속·보호값은 등호로 단정하지 않는다** — 주 경계가 월요일이라 seed를 돌리는 요일에 따라 달라진다.

### 계획에서 방향을 두 번 고쳤다

**① 비밀번호 등급 단정이 rate limit을 터뜨렸다.** 계정 여덟에 로그인을 시도하게 만들었더니
뒤따르는 `yerin 로그인 실패: Request rate limit reached`로 검증이 중단됐다(M-DB-15). 인증 왕복
대신 **저장된 해시를 직접** 보게 바꿨다:

```sql
select p.scody_id, (u.encrypted_password = crypt($1, u.encrypted_password)) as opens
  from auth.users u join public.profiles p on p.id = u.id where p.scody_id = any($2)
```

왕복이 0이고 단정은 **더 강하다** — 엔드포인트의 응답이 아니라 `auth.users`에 실제로 무엇이
들어갔는지를 본다.

**② 데모학원을 학원 격리 검사에 넣었더니 `자기 학원 콘텐츠가 보인다`가 실패했다.** 콘텐츠를
소유하지 않는 설계이므로(세트 총계를 건드리지 않으려고) `ownsContent: false`로 갈랐다. 유출
방향은 다른 학원과 똑같이 검사한다 — 데모 계정이 하필 약한 비밀번호를 쓰므로 **가장 열릴
가능성이 높은 쪽을 검사에서 빼는 것은 거꾸로**다.

### E2E를 타이핑 로그인으로 바꿨다 (이 작업의 가장 큰 항목)

계획을 세울 때 놓쳤다가 설계 검토에서 나온 사실: `e2e/_auth.ts`의 `loginHere()`가 계정 패널의
행을 **클릭**해서 들어간다. 목록을 6개로 줄이면 **로그인 181회가 전부 죽는다**(`seojun` 60 ·
`yerin` 38 · `hanbit.director` 26 · `admin` 25 · `doyun` 14 · 나머지 18).

그래서 폼 입력으로 바꿨다. 지켜야 할 성질 셋을 남겼다.

- 429 재시도 루프(M-DB-15)
- **`login-switch`를 먼저 눌러 세션을 닫는다** — 이 헬퍼는 계정 전환에도 쓰이고 그때 화면에
  폼이 없다(`이미 로그인했어요`). 놓치면 전환 스펙이 조용히 **이전 계정으로** 계속 간다
- 등급별 비밀번호(`demoPassword()`를 `devPassword()` 옆에 두고 `envValue`로 사본을 없앴다)

부수로 얻은 것: 이 헬퍼가 seed의 **열일곱 계정 전부**에 닿는다(전에는 패널에 있는 것만).
그리고 **화면이 실제로 제공하는 길을 검증**하게 됐다 — 클릭 로그인은 사람이 쓸 수 없는 경로였다.

패널은 **읽는 목록**이 됐다: 행에 타이핑할 이메일을 보여 주고 `onPress`가 없다. 비밀번호는
화면에도 번들에도 없고 `.env` 키 이름만 캡션에 있다.

### 지운 테스트 하나

`boundary-flow`의 `중복 아이디로는 가입할 수 없다`를 지웠다. **완화가 아니라 없어진 행동의
테스트를 걷은 것**이다 — 가입이 스코디 아이디를 묻지 않고(필드가 사라졌다) 그 중복을 익명에게
답하지도 않는다. 대신 이 화면이 실제로 하는 검사(이메일 형식)를 `auth-flow`에 새로 넣었다.

### 함께 고친 기존 결함 둘

- **`gen-seed.ts`의 `academyStudents` 필터가 `academyKey`를 보지 않았다.** 좌석 수를 그 배열에서
  파생하는데, 데모 학생 둘이 들어오는 순간 한빛 `contract_seats`가 27 → 29로 부풀어 학원
  대시보드의 좌석 활용률이 **조용히 거짓**을 말한다. E2E가 좌석을 단정하지 않아 드러나지 않았을
  자리다. 지금까지 우연히 맞았던 이유는 `academyRole === 'student'`가 한빛 소속뿐이었기 때문이다.
- **`gen-seed` 출력에 `학원`이 하드코딩 `2`이고 `복습 기록`·`초대`·프로필 합계가 없었다.**
  `verify-rls`가 등호로 보는 값의 절반을 손으로 세게 만들던 자리다. 출력을 확장해 **옮겨 적기**로
  만들었고, 실제로 이번 갱신이 그렇게 됐다(예상값과 전부 일치했다).

### 검증

| 목적 | 결과 |
|---|---|
| `npm run typecheck` · `lint` | 통과 (경고 5는 기준) |
| `npm test` | **306 / 306** (+ `email.test.ts` 8 — 인증 경로의 **첫 단위 테스트**다) |
| `scripts/verify-rls.ts` | **195 / 0** (176 + 비밀번호 등급 8 + 데모 학원 격리 11) |
| `db:verify:records` | **72 / 0** (변경 없음) |
| `verify-note-schedule` | **86 / 0** (변경 없음) |
| `web:export` | 성공 |
| `auth-flow` desktop 단독 | **36 / 36** |

seed 총계는 `gen-seed` 출력을 그대로 옮겼고 **계획의 예상값과 전부 일치**했다: 프로필 31 ·
학원 3 · 반 4 · 배정 7 · 풀이 48 · 노트 18 · 복습 14 · 세트 14 · 문항 192 · 초대 4.

인증 화면의 안전망이 100% Playwright였던 것이 이번에 조금 나아졌다 — `src/session/email.ts`의
세 함수(`normalizeEmail`·`looksLikeEmail`·`loginEmail`)가 단위 테스트로 고정됐다.

### 화면 확인에서 나온 것 셋 (1280·390 × 라이트·다크)

측정으로 확인한 것 — 상단 바 쌍은 두 폭·두 테마 모두 **44px · radius 999 · 좌우 24**이고,
`로그인`은 `surface` 면 + 1px 테두리(라이트 `#d9d2be` · 다크 `#213f3f`)라 다크에서도 버튼으로
읽힌다. `회원가입`만 accent 면이다(라이트 `#20808d` · 다크 `#3aa7b1`, 라벨은 각각 흰색과
`#04211f`). 히어로 CTA는 **52px · radius 12**로 상단 바보다 크고, 390에서 쌍이 `x=169..366`
(오른쪽 여백 24)에 들어간다. **위계가 색이 아니라 크기와 자리로 유지된다**(§8 예외의 조건).

그런데 화면을 실제로 보고 나서야 나온 것이 셋 있다.

**① 학원 대시보드의 머리 지표가 `—`였다.** 데모 원장·선생의 `제출률`·`평균 정답률`이 비어 있고
같은 계정의 `반·학생` 화면은 75%를 말했다(한빛은 95%). 원인: 그 지표의 창은 오른쪽 끝을
`lastCompleteWeek`(지난주 월요일)로 잡는다 — 진행 중인 주를 넣으면 마감이 남아 있어 값이 통째로
내려앉기 때문이다(`src/features/academyStats.ts`). 데모 배정 둘을 `-3`·`+5`로 뒀더니 **둘 다
이번 주**에 들어가 창 밖이었다. `a_demo_1`의 마감을 `-10`으로 물렸다 — 어떤 요일에 seed를 돌려도
완결된 주에 떨어지는 최소값이다(`-4`는 요일에 따라 이번 주에 남는다). 실측: `—` → **100%**.
그 배정에서 나온 노트 둘의 생성일도 제출 뒤로 함께 옮겼다(담은 오답은 그 풀이에서 나온다).

**② 랜딩의 한 문장이 거짓이 됐다.** 히어로 아래 `이미 스코디 아이디가 있으면 로그인할 수
있어요.`(`LOGIN_AVAILABLE`, D-175가 둔 문장) — 로그인이 이메일을 받으므로 사실이 아니다.
`이미 계정이 있으면 이메일로 로그인할 수 있어요.`로 고쳤다. 개인정보처리방침의 수집 항목에도
이메일 주소를 더했다(`가입·인증:` 줄) — 실제로 받게 됐으므로.

**③ 한 묶음 안에서 기준이 갈렸다.** `계정 정보`의 칸 순서가 `이메일 → 이름 → 비밀번호`라 이메일과
비밀번호가 떨어져 있었고(비밀번호 관리자는 두 칸을 한 쌍으로 본다), `이름`·`비밀번호`는 라벨과
**똑같은 placeholder**를 갖고 있었다 — 이메일 칸을 placeholder 없이 둔 근거(§15 "같은 말을 두 번
쓰지 않는다")가 바로 옆에서 지켜지지 않았다. 순서를 `이름 → 이메일 → 비밀번호`로 바꾸고
(이름이 두 경로가 함께 묻는 유일한 칸이라 맨 위가 자리가 안정적이다) 되풀이 placeholder 둘을 지웠다.

### 데모 6계정 화면 확인 (로그인 → 첫 화면)

| 계정 | 첫 화면 | 확인한 것 |
|---|---|---|
| `student1` | `/student` | `5일 연속 · 오늘 3문항을 풀면 이어져요` · `오늘 다시 볼 오답 3개` · `마친 학습 2/2` · 기록 탭에 `10일 공부` · `다가오는 기록` 3개 · 최근 4주 |
| `student2` | `/student` | `아직 시작하지 않은 과제가 있어요` · 학원 과제 `비판적 읽기 연습` · `오늘 다시 볼 오답 1개` · `1/2 완료` |
| `parent1` | `/parent` | 자녀 `유하람 · 8월에 8일 공부했어요` |
| `parent2` | `/parent` | 자녀 `노도현 · 8월에 4일 공부했어요 · 안 낸 과제 1개` |
| `academy1`(원장) | `/academy` | `반 1개 · 학생 2명` · 제출률 100% · 반 목록에 `데모 고1 국어 · 데모 선생 · 2명 · 4건 · 75%` |
| `academy2`(선생) | `/academy` | `스코디 데모학원 · 선생님` · `안 낸 학생 1명` · **담당 반이 자기 것으로 보인다** |

**빈 화면이 하나도 없다.** 선생 계정의 담당 반이 특히 중요했다 — 세션이 선생에게 `teacherId`가
자기인 반만 돌려주므로 담당을 주지 않으면 그 계정의 반·학생 화면이 통째로 빈다.

### 꺼진 빌드 번들 대조

**처음 두 번은 잘못 쟀다.** `.env`의 `EXPO_PUBLIC_ENABLE_DEV_LOGIN=1`이 셸 환경변수를 덮어서
사실상 켜진 빌드를 검사했고(판정 근거: `테스트 계정` 12회 · `카카오로 로그인` 2회), 세 번째는
Metro 캐시가 살아 산출물이 **바이트 수까지 동일**했다. `.env`를 임시로 고치고 `--clear`로 다시
빌드해서야 실제 꺼진 빌드가 나왔다.

`\uXXXX`를 디코드한 뒤 센 결과(D-158이 실측한 함정 — 번들의 한글은 이스케이프된다):

```
유하람 0 · 노도현 0 · 유선경 0 · 노태식 0 · 데모 원장 0 · 데모 선생 0
스코디 데모학원 0 · student1@scody.test 0 · 010-6000- 0 · 데모 비밀번호 0
(옛 값도 그대로 0: doyun · 한빛 원장 · 010-3000-0001 · @peer.scody.test)
남아야 하는 것: 카카오 로그인은 아직 연결되지 않았어요 · 스코디에 로그인 · 계정 복구는 준비 중이에요
```

**데모 식별자와 비밀번호가 전부 0회** — D-145의 상수 접기가 새 배열에서도 작동하고, 비밀번호는
접기에 기대지도 않는다(접두어가 없어 애초에 클라이언트에 없다).

### E2E 3뷰포트 (캐시를 비운 `dist`로 다시 돌린 최종)

| 뷰포트 | 결과 |
|---|---|
| desktop | **197 통과 / 6 실패 — 전부 기준 실패** |
| tablet | **197 / 6 — 전부 기준 실패** |
| mobile | 196 / 7 — 기준 6 + 하나 |

합 **590 통과 / 19 실패 = 기준 18 + 환경 1**. 추가 하나(`admin-flow:291`)는 3뷰포트 단독
**3/3 통과** — 전체 스위트에서만 깨지는 교차 스펙 경합이다(M-DB-15). **새 회귀는 없다.**

`loginHere`를 타이핑으로 바꾼 것이 181개 로그인 전부에 걸리는 변경이었는데, 계정 전환 스펙에서
하나도 실패하지 않았다 — `login-switch` 선처리를 넣은 것이 그 자리다.

(오염된 `dist`로 돌린 앞 실행은 desktop·tablet 각 11건 · mobile 10건이었고, 그 차이 전부가
아래 `스스로 만든 함정` 절의 캐시 문제였다. 캐시를 비우고 다시 빌드하자 5건이 그대로 사라졌다.)

### 한 번 헷갈렸던 것

dev 서버를 막 띄운 직후 브라우저 로그인이 한 번 실패했다. 토큰 호출은 **200**인데 화면은
`이메일이나 비밀번호를 확인해 주세요.`였고 동시에 `이미 로그인했어요`가 떠 있었다 —
`onAuthStateChange`의 `hydrate`는 성공했고 `openSession`의 인라인 조회는 실패한 상태다. 같은
조건을 세 계정으로 다시 재면 3/3 통과하고 E2E도 36/36이라, **첫 컴파일 중의 일시적 경합**으로
판단했다. 제품 결함이라면 E2E에서 재현됐을 것이다.

### 스스로 만든 함정 하나 — 꺼진 빌드 검사가 캐시를 오염시켰다

마지막 전체 E2E에서 **desktop·tablet 모두 같은 5건**이 깨졌다(auth-flow `226`·`370`·`437`·`458`·
`503`). 흔들림이 아니라 결정적이었고, 몇 분 전 같은 파일을 단독으로 돌렸을 때는 36/36이었다.

`.env`는 정상이었다(`EXPO_PUBLIC_ENABLE_DEV_LOGIN=1` · 비밀번호 있음). 빌드 산출물을 열어 보니:

```
dist:  '지금은 로그인 화면의 테스트 계정으로…' 1회   ← signup.tsx는 새로 컴파일됐다
       '테스트 계정' 12회                          ← DemoAccounts.tsx도 새로 컴파일됐다
       유하람 0회 · student1@scody.test 0회         ← DEV_ACCOUNTS는 빈 배열로 접혀 있다
```

**앞서 꺼진 빌드를 검사하려고 `--clear`로 export한 것이 Metro 캐시에 `devAccounts.ts`의 꺼진
판본을 남겼고**, 뒤이은 `npm run web:export`(`--clear` 없음)가 그것을 재사용했다. 모듈 하나만
꺼진 상태라 증상이 정확히 다섯 곳에 나왔다: 계정 배열이 비어 패널이 `null`이 되고(D-141의
`0개면 두지 않는다`), `DEV_LOGIN_ENABLED`가 모듈 경계를 넘어 상수로 접히지 못해 `SIGNUP_PENDING`이
**런타임에 짧은 변형**을 골랐다(두 리터럴은 산출물에 다 있었다) — `_auth.ts`가 긴 변형을 기대하는
네 곳과 `signup-error-login` 하나가 함께 깨졌다.

**코드 결함이 아니라 빌드 산출물 문제다.** 그리고 그 5건은 이 변경이 실제로 건드린 자리라, 원인을
캐시로 좁히지 않았다면 내 코드 탓으로 읽었을 것이다.

교훈: **꺼진 빌드 대조는 별 디렉터리로 빼도 캐시는 공유한다.** 그 검사를 한 뒤에는 `dist`를
`--clear`로 다시 만들어야 하고, 이 절차를 검증 표에 적어 둔다.

### 마지막 확인에서 하나 더 — 검증기가 오염을 잡았다

E2E 직후 재시드 없이 `verify-rls`를 돌렸더니 **192/3**이었다: `콘텐츠 14세트` · `문항 192개` ·
`풀이 48건`이 어긋났다. 원인은 E2E다 — 스위트는 테스트마다 재시드하지만 **마지막 테스트 뒤에는
하지 않는다**. `admin-flow`의 문제 등록 테스트가 만든 콘텐츠가 그대로 남아 있었다.
`npm run db:verify`(재시드 포함)로 돌리면 **195/0**이다.

이것이 D-184에 적은 판단이 실제로 작동한 예다: 데모 계정은 약한 비밀번호를 쓰고 공개 사이트가
이 DB를 가리키므로(M-DB-7) 누가 들어와 행을 만들면 **등호 단정이 그 오염의 유일한 감지 수단**이다.
`>=`로 풀었다면 이 세 줄이 조용히 통과했을 것이다.
...
| `npm run typecheck` · `lint` | 통과 (경고 5는 기준) |
| `npm test` | **306 / 306** |
| `npm run db:verify`(재시드 + RLS) | **195 / 0** |
| `db:verify:records` | **72 / 0** |
| `verify-note-schedule` | **86 / 0** |
| `expo export --clear` | 성공 · 켜진 빌드에 계정 배열 있음 |
| 꺼진 빌드 번들 대조 | 데모 식별자·비밀번호 **전부 0회** |
| Playwright 3뷰포트 | **590 / 19** = 기준 18 + 환경 1(단독 3/3 통과) |
| 브라우저 | 1280·390 × 라이트·다크 4조합 + 데모 6계정 로그인 |

작업 디렉터리를 정리했다: `.firecrawl/`(조사에 쓴 8개 md)와 임시 확인 스크립트를 지웠고,
근거 이미지 8장은 `docs/evidence/auth-*.png`에 남겼다.
